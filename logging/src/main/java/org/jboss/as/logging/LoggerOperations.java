...
final class LoggerOperations {
    ...
    static final AttributeDefinition[] ROOT_LOGGER_ATTRIBUTES = {
            FILTER,
            LEVEL,
            HANDLERS
    };

    static final AttributeDefinition[] LOGGER_ATTRIBUTES = {
            FILTER,
            LEVEL,
            HANDLERS,
            USE_PARENT_HANDLERS
    ...
    static final class LoggerAddOperationStepHandler extends LoggingAddOperationStepHandler {
        ...
        public void performRuntime(final OperationContext context, final ModelNode operation, final LogContextConfiguration logContextConfiguration, final String name, final ModelNode model) throws OperationFailedException {
            final String loggerName = getLogManagerLoggerName(name);
            LoggingLogger.ROOT_LOGGER.tracef("Adding logger '%s' at '%s'", name, operation.get(OP_ADDR));
            LoggerConfiguration configuration = logContextConfiguration.getLoggerConfiguration(loggerName);
            if (configuration == null) {
        ...
        public void performRollback(final OperationContext context, final ModelNode operation, final LogContextConfiguration logContextConfiguration, final String name) {
            if (logContextConfiguration != null) {
                final String loggerName = getLogManagerLoggerName(name);
                LoggingLogger.ROOT_LOGGER.tracef("Rolling back added logger '%s' at '%s'", name, operation.get(OP_ADDR));
     ...
     */
    public static LoggerAddOperationStepHandler ADD_ROOT_LOGGER = new LoggerAddOperationStepHandler(ROOT_LOGGER_ATTRIBUTES);

    /**
     * A step handler to add a logger.
     */
    public static LoggerAddOperationStepHandler ADD_LOGGER = new LoggerAddOperationStepHandler(LOGGER_ATTRIBUTES);

    /**
     * Write attribute handlers.
     */
    public static LoggerWriteAttributeHandler ROOT_LOGGER_WRITE_HANDLER = new LoggerWriteAttributeHandler(ROOT_LOGGER_ATTRIBUTES);
    public static LoggerWriteAttributeHandler LOGGER_WRITE_HANDLER = new LoggerWriteAttributeHandler(LOGGER_ATTRIBUTES);

    /**
     * A step handler to remove a logger
     */
    public static LoggingRemoveOperationStepHandler REMOVE_LOGGER = new LoggingRemoveOperationStepHandler() {
        ...
        protected void performRollback(final OperationContext context, final ModelNode operation, final LogContextConfiguration logContextConfiguration, final String name, final ModelNode originalModel) throws OperationFailedException {
            ...
            if (loggerName.equals(ROOT_LOGGER_NAME)) {
                ADD_ROOT_LOGGER.performRuntime(context, operation, logContextConfiguration, name, originalModel);
            } else {
                ADD_LOGGER.performRuntime(context, operation, logContextConfiguration, name, originalModel);
            }
        }
    };
    ...
    public static LoggerUpdateOperationStepHandler ADD_HANDLER = new LoggerUpdateOperationStepHandler() {
        ...
        public void performRuntime(final OperationContext context, final ModelNode operation, final LoggerConfiguration configuration, final String name, final ModelNode model) throws OperationFailedException {
            ...
            LoggingLogger.ROOT_LOGGER.tracef("Adding handler '%s' to logger '%s' at '%s'", handlerName, getLogManagerLoggerName(loggerName), operation.get(OP_ADDR));
        ...
        public void performRollback(final OperationContext context, final ModelNode operation, final LoggerConfiguration configuration, final String name, final ModelNode originalModel) throws OperationFailedException {
            ...
            final String loggerName = getLogManagerLoggerName(name);
            if (configuration.getHandlerNames().contains(handlerName)) {
                LoggingLogger.ROOT_LOGGER.tracef("Rolling back added handler '%s' to logger '%s' at '%s'", handlerName, getLogManagerLoggerName(loggerName), operation.get(OP_ADDR));
    ...
    public static LoggerUpdateOperationStepHandler REMOVE_HANDLER = new LoggerUpdateOperationStepHandler() {
        ...
        public void performRollback(final OperationContext context, final ModelNode operation, final LoggerConfiguration configuration, final String name, final ModelNode originalModel) throws OperationFailedException {
            ...
            final String loggerName = getLogManagerLoggerName(name);
            if (!configuration.getHandlerNames().contains(handlerName)) {
                LoggingLogger.ROOT_LOGGER.tracef("Rolling back removed handler '%s' to logger '%s' at '%s'", handlerName, getLogManagerLoggerName(loggerName), operation.get(OP_ADDR));
                configuration.addHandlerName(handlerName);
            }
        }
    };

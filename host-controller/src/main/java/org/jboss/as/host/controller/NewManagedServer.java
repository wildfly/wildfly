 ...
 */
public final class NewManagedServer {
    ...
    //TODO get rid of oldDomainModel and oldHostModel
    public NewManagedServer(final String serverName, final DomainModel oldDomainModel, final HostModel oldHostModel,
            final ModelNode domainModel, final ModelNode hostModel,
        ...
        assert oldDomainModel != null : "domainModel is null";
        assert oldHostModel   != null : "hostModel is null";
        ...
        ServerFactory.combine(oldDomainModel, oldHostModel, serverName, updateList);

        ServerElement oldServer = oldHostModel.getServer(serverName);
        if (oldServer == null) {
            throw new IllegalStateException("Server " + serverName + " is not listed in Host");
        }
        final ModelNode server = hostModel.get(SERVER, serverName);
        if (server == null) {
            throw new IllegalStateException("Server " + serverName + " is not listed in Host");
        }

        //String serverGroupName = server.getServerGroup();
        final String serverGroupName = server.get(GROUP).asString();
        ServerGroupElement serverGroup = oldDomainModel.getServerGroup(serverGroupName);
        if (serverGroup == null)
        ...
        this.portOffset = server.get(SOCKET_BINDING_GROUP_NAME).isDefined() ? serverGroup.getSocketBindingPortOffset() : server.get(SOCKET_BINDING_PORT_OFFSET).asInt(0);

        JvmElement serverVM = oldServer.getJvm();
        ...
        JvmElement hostVM = oldHostModel.getJvm(ourVMName);

        jvmElement = new JvmElement(groupVM, hostVM, serverVM);

        Map<String, String> properties = new HashMap<String, String>();
        properties.putAll(oldDomainModel.getSystemProperties().getProperties());
        properties.putAll(serverGroup.getSystemProperties().getProperties());
        properties.putAll(oldHostModel.getSystemProperties().getProperties());
        //properties.putAll(server.getSystemProperties().getProperties());
        properties.putAll(getSystemProperties(server));
    ...
    private Map<String, String> getSystemProperties(final ModelNode model) {
        if (model.has(SYSTEM_PROPERTY) && model.get(SYSTEM_PROPERTY).isDefined()) {
            List<Property> props = model.get(SYSTEM_PROPERTY).asPropertyList();
            if (props.size() >= 0) {
                Map<String, String> result = new HashMap<String, String>();
                for (Property prop : props) {
                    result.put(prop.getName(), prop.getValue().asString());
                }
                return result;
            }
        }

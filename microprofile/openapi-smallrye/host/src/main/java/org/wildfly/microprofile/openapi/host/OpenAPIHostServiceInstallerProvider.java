...
public class OpenAPIHostServiceInstallerProvider implements HostServiceInstallerProvider, Predicate<RuntimeCapability> {
    ...
    public ResourceServiceInstaller getServiceInstaller(String serverName, String hostName) {
        return new ResourceServiceInstaller() {
            ...
            public Consumer<OperationContext> install(OperationContext context) {
                ...
                RuntimeCapability<?> hostCapability = context.getResourceRegistration().getCapabilities().stream().filter(OpenAPIHostServiceInstallerProvider.this).findFirst().orElse(null);
    ...
    @Override
    public boolean test(RuntimeCapability capability) {
        return capability.getName().equals(Host.SERVICE_DESCRIPTOR.getName());
    }
}

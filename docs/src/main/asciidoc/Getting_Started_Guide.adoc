...
WildFly {wildflyVersion} is the latest release in a decades-long series of open-source
application server offerings. WildFly is an exceptionally fast,
lightweight and powerful implementation of Jakarta
and MicroProfile specifications, providing advanced security, clustering, management and observability capabilities. WildFly's modular architecture, built on the
JBoss Modules and JBoss Modular Service Container projects, enables services on-demand when your
application requires them.

WildFly currently offers two stable variants: standard WildFly, offering the EE 11 specifications from Jakarta, and a new variant providing EE 10 that is offered temporarily to give users time to transition to EE 11 while still getting new features and bug fixes. We also offer a tech preview variant, WildFly Preview, used to showcase things in the works for future releases of standard WildFly. For more on these options, please see the xref:Different_Flavors_of_WildFly{outfilesuffix}[Different Flavors of WildFly] guide.

[[standards_support]]
== Standards Support

The table below lists the versions of the Jakarta technologies available in WildFly and WildFly EE 10, along with whether the technology is present in our primary xref:#wildfly-configurations[out-of-the-box server configuration profiles].
...
|Jakarta Annotations |3.0 |2.1 |X |X

|Jakarta Authentication  |3.1 |3.0 |X |X

|Jakarta Authorization  |3.0 |2.1 |X |--

|Jakarta Batch |2.1 |2.1 |X |--

|Jakarta Concurrency  |3.1 |3.0 |X |X

|Jakarta Connectors  |2.1 |2.1 |X |X

|Jakarta Contexts and Dependency Injection  |4.1|4.0 |X |X

|Jakarta Data^xref:#spec_note_1[1]^
| 1.0 |1.0 |X | X

|Jakarta Debugging Support for Other Languages  |2.0 |2.0 |X |X

|Jakarta Dependency Injection  |2.0 |2.0 |X |X

|Jakarta Enterprise Beans  |4.0 |4.0 |X |X(Lite)

|Jakarta Expression Language  |6.0 |5.0 |X |X

|Jakarta Faces  |4.1 |4.0 |X |X

|Jakarta Interceptors  |2.2 |2.1 |X |X

|Jakarta JSON Binding  |3.0 |3.0 |X |X

|Jakarta JSON Processing  |2.1 |2.1 |X |X

|Jakarta Mail  |2.1 |2.1 |X |X

|Jakarta Messaging  |3.1 |3.1 |X |--

|Jakarta MVC^xref:#spec_note_2[2]^ |2.1|2.1|--|--

|Jakarta Pages  |3.1 |3.1 |X |X

|Jakarta Persistence  |3.2 |3.1 |X |X

|Jakarta RESTful Web Services  |4.0 |3.1 |X |X

|Jakarta Security  |4.0 |3.0 |X |X

|Jakarta Servlet  |6.1 |6.0 |X |X

|Jakarta SOAP with Attachments^xref:#spec_note_3[3]^ |3.0 |3.0 |X |X

|Jakarta Standard Tag Library  |3.0 |3.0 |X |X

|Jakarta Transactions  |2.0 |2.0 |X |X

|Jakarta Validation  |3.0 |3.0 |X |X

|Jakarta WebSocket  |2.2 |2.1 |X |X

|Jakarta XML Binding^xref:#spec_note_3[3]^  |4.0 |4.0 |X |X

|Jakarta XML Web Services^xref:#spec_note_3[3]^  |4.0 |4.0 |X |X
|=======================================================================

Notes:

. [[spec_note_1]]Jakarta Data is provided at `community` xref:Admin_Guide.adoc#Feature_stability_levels[stability]. While it is not part of EE 10, the WildFly EE 10 variant does support it, although it does not provide it in any out-of-the-box configuration.
. [[spec_note_2]]Jakarta MVC is provided at `preview` xref:Admin_Guide.adoc#Feature_stability_levels[stability].
. [[spec_note_3]]This specification is optional in EE 10 and was removed from EE 11. However, WildFly still supports it.

[NOTE]
====
WildFly Preview often (but not always) provides implementations of different versions of the specifications listed above than standard WildFly does. See xref:Different_Flavors_of_WildFly.adoc#wildfly-preview-ee[EE Support in WildFly Preview] for current details.
====


[WARNING]

WildFly's default configuration provides EE Web Profile support and thus
...
profiles do provide all features required by the EE Full Platform. If you
...
|MicroProfile OpenAPI 4.1 |-- |X
|MicroProfile Reactive Messaging 3.0 |-- |--
|MicroProfile Rest Client 4.0|X |X
|MicroProfile Streams Operators 3.0 |-- |--
|MicroProfile Telemetry 2.1|--|X
|=======================================================================

[[requirements]]
== Requirements

* Java SE {min-java-version} or later. We recommend that you use the latest available update
of the current long-term support Java release, Java SE {recommended-java-version}. For the WildFly EE 10 variant, Java SE {recommended-java-version-legacy} is recommended.


[[installation-options]]
== Installation Options

There are a number of ways you can install WildFly, including unzipping our traditional download zip, provisioning a custom installation using the `wildfly-maven-plugin` or the Galleon CLI, or building a bootable jar. There are also the link:Different_Flavors_of_WildFly{outfilesuffix}[standard and EE 10 variants of WildFly and the tech-preview WildFly Preview variant].
...
[[creating-new-jakarta-ee-application]]
=== Creating a New Jakarta EE Application

The Jakarta EE Starter https://start.jakarta.ee simplifies the creation of new projects based on Jakarta EE.
Following some simple steps can get a basic Jakarta EE application with WildFly.

This tutorial can be a complement to the guide: https://www.wildfly.org/get-started/ .
...
directory for automatic detection and deployment into
...
|data |Persistent information written by the Host Controller to survive a restart of the Host Controller.

|lib/ext |Location for installed library jars referenced by applications
using the Extension-List mechanism

|log |Location where the Host Controller process writes its logs. The
Process Controller, a small lightweight process that actually spawns the Host Controller process and any Application Server processes also
writes a log here.
...
** EE Web Profile configuration with
the required technologies plus those noted in the table above.

* `standalone-ha.xml`
** EE Web Profile configuration with high availability

* `standalone-full.xml`
** EE Platform configuration including all the required technologies

* `standalone-full-ha.xml`
** EE Platform configuration with high availability
...
technologies that Jakarta RESTful Web Services applications commonly use to integrate with
external services.
...
* `domain.xml`
** EE full and web profiles available with or
...
authentication and the files are reloaded if they have changed. Therefore, you do not need to re-start the server after adding a new user.
...
////
    TODO https://redhat.atlassian.net/browse/WFCORE-5532
...
[NOTE]
====
If no host or port information provided, it will default to
localhost:9990.
====

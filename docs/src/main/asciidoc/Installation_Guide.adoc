...
The WildFly project now produces three appserver variants, link:Different_Flavors_of_WildFly{outfilesuffix}[standard WildFly, WildFly EE 10 and WildFly Preview],
so you'll want to decide which is right for you. For most users the standard WildFly variant is the right choice,
but if you need to stick with EE 10 for a little while longer, try WildFly EE 10, and if you'd like a technical preview look at what's coming in the future, try out WildFly Preview.
...
* You want to rely on a traditional EE application deployment style, using a server provisioned without any deployments with the server then instructed to install deployments.
* You want a server that offers all Jakarta and MicroProfile specifications.
...
* The link:Application_Development_Guide{outfilesuffix}[Application Development Guide] shows you how to build
...
Some applications don't need a fully featured server supporting all operating modes and containing all Jakarta and MicroProfile capabilities.
...
* Your application requires only a subset of the Jakarta or MicroProfile APIs
(although Galleon can provision a server that supports the full set of Jakarta and MicroProfile platform APIs).
...
* Your application requires only a subset of the Jakarta or MicroProfile APIs
(although the WildFly Maven Plugin can provision a server that supports the full set of Jakarta and MicroProfile platform APIs).
* You are concerned by server installation size and memory footprint.
* You are only using standalone operating mode (with support for High Availability or not).
* Your server instances will contain one or more application deployments.

...
* We append `env.` onto the original name, i,e, `env.my.example-expr`
and check if there is an environment variable with that
name. If there is, we use its value as the result of the resolution.
If not, we continue checking the next location.
* All attempts at resolution failed. Since our example provided no
default, the expression could not be resolved. If we had specified
a default in the expression, for example`${my.example-expr:hello}`,

...
This will create a context on the management interface from which metrics can be scraped. For example, the above configuration would result in a URL that looks like `http://localhost:9090/metrics`. If you have not disabled the WildFly Metrics subsystem, you will not be able to use `/metrics` as your context, as that is already configured. To disable the WildFly Metrics Subsystem, see the `remove` commands above. Alternatively, you can specify a different context, e.g., `/prometheus`, though
the performance concerns above are still valid.
...
Unlike the previous metrics systems, this new extension exposes an API (see https://micrometer.io[Micrometer]) to applications in order to allow developers to record and export metrics out of the box. To do so, application developers will need to inject a `MeterRegistry` instance:

[source,java]
-----
package org.wildfly.micrometerdemo;
...
====
Generally speaking, applications deployed to an application server like WildFly run in isolation. While that remains true when using the Micrometer subsystem, there is an important caveat. WildFly creates a single instance of each configured `MetricRegistry` for the entire system, so there will be single point from which metrics are pushed or pulled. This means that metrics from all applications deployed to a given server will be aggregated and published together. If this mingling of metrics poses concerns for your environment, consider deploying sensitive applications to isolated environments.
====

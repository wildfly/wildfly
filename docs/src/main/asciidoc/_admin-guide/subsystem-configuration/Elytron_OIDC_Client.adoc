...
OpenID Connect allows the client to configurate application based URIs that will
trigger the OpenID Provider to log out the user.
...
----
{
  ...
  "logout-callback-path" : "http://localhost:8080/customer-portal/goodbye-page",
   "credentials" : {
      "secret" : "234234-234234-234234"
   }
}
...
* `post-logout-uri` : An absolute URI.  It will be called after logout has been executed.  It is used by RP-Initiated logout only.

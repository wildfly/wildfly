...
Three separate logout mechanisms are supported:

* RP-Initiated Logout starts when the user requests logout from the application. The application invalidates the local session and redirects the user to the OpenID Provider `end_session_endpoint`.
* Front-Channel Logout is a browser-based mechanism in which the OpenID Provider notifies registered clients through the user's browser.
* Back-Channel Logout is an application-to-application mechanism in which the OpenID Provider sends a logout token directly to registered client callback endpoints.

Logout is enabled when at least one of `logout-path`, `logout-callback-path`, or `post-logout-redirect-uri` is configured.
...
* `logout-path`: Optional relative path (starting with `/`) for the application endpoint that starts RP-initiated logout. Configure only when users initiate logout from the application; not used by front-channel or back-channel logout. No default value. Must match the incoming request path exactly.
* `logout-callback-path`: Relative path or absolute `http`/`https` URI where the OpenID Provider invokes front-channel or back-channel logout. Only the path component is matched against the incoming request, using exact path equality. No default value. Must be registered with the OpenID Provider. Optional attribute, but required in practice for front-channel or back-channel logout to work.
* `post-logout-redirect-uri`: Optional absolute URI sent to the provider `end_session_endpoint` as `post_logout_redirect_uri` during RP-initiated logout. Must be registered with the OpenID Provider.
* `logout-session-required`: When true (default), front-channel logout validates the provider sid against the ID token sid claim and enforces iss. When false, front-channel logout skips those checks and invalidates the local session on the callback request. For back-channel logout to be executed, the value must be `true`. When value is `false`, the `back-channel logout is rejected with HTTP 400.

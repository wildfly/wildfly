...
proxies from existing objects. WildFly's Context Services are also used to
propagate the context from an EE application invocation thread to
the threads internally used by the other Jakarta Concurrency utilities.
...
The Managed Thread Factory allows EE applications to create new
threads. WildFly Managed Thread Factory instances may also, optionally,
use a Context Service instance to propagate the EE application
...
The Managed Executor Service is the Jakarta Concurrency adaptation of the Java SE
Executor Service, providing to EE applications the functionality of
asynchronous task execution. WildFly is responsible for managing the
...
The Managed Scheduled Executor Service is the Jakarta Concurrency adaptation of the Java
SE Scheduled Executor Service, providing to EE applications the

...
Handler element allows to define server specific handlers, distinct from filters or predicates. Currently list of available is limited to *file* and *reverse-proxy*.

=== File

File handler is terminating handler for incoming request. It allows configuration of static content that will be served by server if certain criteria are met - ie. default access to `/` of server.
...
|name|Simply a name/alias by which file handler will be referenced in server.
|path|Absolute path to resource being served via this file handler.
...
Reverse proxy handler allow to configure server in reverse proxy role. For example please check out <<Undertow_using_as_a_Load_Balancer, Undertow as Load Balancer>>.
...
|name|Simply a name/alias by which proxy handler will be referenced in server.
...
Host element represents host/connection that will be target of requests routed via reverse proxy.

[cols=",",options="header"]
|=======================================================================
|Attribute |Description

|name|Simply a name/alias by which host will be referenced in server.

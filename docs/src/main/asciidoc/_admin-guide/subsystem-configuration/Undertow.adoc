...
When statistics are enabled in the Undertow subsystem,
session-related metrics are collected for each deployed web application.
These metrics can be useful for monitoring usage and capacity tuning.
...
WARNING: Collecting metrics does incur a small performance cost.

Session statistics are available at the deployment level via the management model:

[source]
----
/deployment=<deployment-name>/subsystem=undertow:read-resource(include-runtime=true)
----

Or to obtain value of a specific metric:
...
The following session statistics attributes are available:
...
|highest-session-count |The maximum number of concurrent active sessions that have been active at any point in time
|max-active-sessions |The maximum number of active sessions allowed, as configured via `<max-active-sessions>` in `jboss-web.xml` (or -1 if unlimited)
|rejected-sessions |The number of sessions that were rejected
|sessions-created |The total number of sessions that have been created
|session-avg-alive-time |The average time (in seconds) that expired sessions had been alive
|session-max-alive-time |The longest time (in seconds) that an expired session had been alive
|======================================================

== Handlers

A handler element allows you to define server specific HTTP request handlers, distinct from filters or predicates. Currently the available handler types are limited to:
...
A file handler is a terminating handler for an incoming request. It allows configuration of static content that will be served in response to a request for the location to which it is mapped if not otherwise handled.
...
|name|Simply a name by which the file handler will be referenced in other parts of the server configuration.
|path|Absolute path to resources being served via this file handler.
|cache-buffer-size|Size of I/O buffer in bytes.
|cache-buffers|Number of buffers.
|directory-listing|Enable directory listing in output.
|follow-symlink|Enable following of symlinks in FS.
|safe-symlink-paths|Comma separated list of paths that are safe to be targets of symbolic links.
...
A reverse proxy handler allows you to configure the server in a reverse proxy role. For example, see <<Undertow_using_as_a_Load_Balancer, Undertow as Load Balancer>>.
In such role server acts as frontend for content serving endpoints.

[source,xml,options="nowrap"]
----
<handlers>
   <reverse-proxy name="proxy-name">
      <host name="proxy-host"/>
   </reverse-proxy>
</handlers>
----
...
|name|Simply a name by which the proxy handler will be referenced in other parts of the server configuration.
|connections-per-thread|Number of connections that will be maintained to backend servers, per IO thread.
|session-cookie-names|Comma separated list of session cookie names.
|problem-server-retry|Delay in seconds between connection attempts to server that is down.
|max-request-time|Maximum life time, in milliseconds, of a request, before it is terminated.
|request-queue-size|Number of requests that can linger in queue if connection pool is full. Overflowing requests will be terminated with 503 response.
|cached-connections-per-thread|Number of connection per thread that will be kept alive indefinitely.
|connection-idle-timeout|Time in milliseconds connection can be inactive before it is reclaimed - unless number of active/alive connections is below or equal `cached-connections-per-thread`
|max-retries|Number of times to attempt to retry a request if it fails. Note that if a request is not considered idempotent then it will only be retried if the proxy can be sure it was not sent to the backend server).
|reuse-x-forwarded-header|Append(reuse existing) remote hop to x-forwarded-* headers, if not, previous value is discarded and remote hop becomes the value.
|rewrite-host-header|Set host header to connection remote end and create x-forwarded-host header. This is done to adhere to host header validation on remote end.
...
A host element represents host/connection that will be the target of requests routed via the reverse proxy.

[cols=",",options="header"]
|=======================================================================
|Attribute |Description

|name|Simply a names by which host will be referenced in other parts of the server configuration
...
|scheme|What kind of scheme is used

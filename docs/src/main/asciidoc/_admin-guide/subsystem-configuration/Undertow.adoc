...
|obfuscate-session-route | set this to true to indicate the instance-id and server
name should be both obfuscated in routing. This prevents instance-id and server
name data from being sent across HTTP connections when serving remote requests with
the HTTP invoker

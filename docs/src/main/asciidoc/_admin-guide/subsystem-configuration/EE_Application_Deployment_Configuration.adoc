...
deployment behaviour for EE Applications.

[[global-modules]]
== Global Modules

Global modules are a set of JBoss Modules that will be added as
dependencies to the JBoss Modules module of every deployment. Such
dependencies allow deployments to see the classes exported by
the global modules.

Each global module is defined through the `module` resource. Here's an example
...
Annotations included in EE deployments.
...
standard EE XML descriptors. If not configured this defaults to

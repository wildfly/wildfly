...
[[transactions-subsystem-suspension]]
==== Transaction Recovery and Server Suspension

While the server is suspending, the transaction subsystem
waits for all _in-flight_ transactions to reach a terminal state.
During that time the server refuses to start any new transaction.
However, transactions already in progress continue to be serviced;
e.g. the server will accept any incoming remote call
which carries the context of a transaction already started at the suspending server.

If the `transactions-recovery-graceful-shutdown` attribute is set to `wait` in the
transactions subsystem, the server will also wait for _in-doubt_ transactions that
require recovery to be resolved before fully suspending. By *default* this attribute
is set to `ignore`, meaning the recovery manager will not delay the suspension of the
server while waiting for such transactions to complete.
To enable this behavior, set the attribute in the transactions subsystem section of `standalone.xml`:
...
<recovery-environment socket-binding="txn-recovery-environment"
    ...
    transactions-recovery-graceful-shutdown="wait"/>
----
...
[NOTE]
====
This behavior *requires* the Jakarta Enterprise Beans graceful shutdown functionality to be enabled,
as described in <<transactions-and-Jakarta-Enterprise-Beans>>.
====

[NOTE]
====
Heuristic transactions do not delay a graceful shutdown.
However, they are reported through the standard logging mechanisms and
require manual reconciliation after the server has shut down.
====

WARNING: In cloud environments, where pods are ephemeral and logs are not retained,
specify a persistent and secure location for saving the object store and report logs.

...
* *Transactions* - While the server is suspending, the transaction subsystem
...
+
If the `transactions-recovery-graceful-shutdown` attribute is set to `true` in the
...
is `false`, meaning the recovery manager will not delay the suspension of the server
while waiting for such transactions to complete.
...
<recovery-environment socket-binding="txn-recovery-environment"
    ...
    transactions-recovery-graceful-shutdown="true"/>
----
+
Or use the CLI:
+
...
----
/subsystem=transactions:write-attribute(name=transactions-recovery-graceful-shutdown, value=true)
----
+
[NOTE]
====
...
====
+
WARNING: In cloud environments, where pods are ephemeral and logs are not retained, specify a persistent and secure location for saving the object store and report logs.

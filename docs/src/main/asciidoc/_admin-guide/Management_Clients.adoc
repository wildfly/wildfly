...
[[http-management-endpoint-securing]]
== Endpoint User Authentication

For in depth explanation of security, please refer to <<WildFly_Elytron_Security.adoc#default-management-authentication-configuration, Elytron default management domain>>.

Management endpoint need way to authenticate and authorize or reject incoming requests. Thus management interface have mandatory requirement to have authentication mechanism configured.
Server running in domain mode use this mechanism to register as part of domain.

=== Realm and Domain definitions

Example definition of realm can look as follows:
...
Note, that aside for regular properties backed(DIGEST) realm, there is one additional realm **local**. This is special type of realm(identity-realm) which defines a single pre-configured static identity. 
This identity is assigned to any action performed within "local" scope - ie, as user connect via CLI to local server.
Corresponding definition of domain can look as follows:

[[http-management-endpoint-securing-super-user, Example Management Domain]]
[source,xml,options="nowrap"]
----
<security-domains>
   [...]
...
This particular example create domain which is backed by **ManagementRealm** as source of credentials and fallback to **local** in case those are not found in **ManagementRealm**.


=== Role mapping

Additionally, server need mappers to:

* set default permissions
* create Super User role


Corresponding definition of mappers can look as follows:
...
To tie domain to specific authentication mechanism, server needs authentication factory. For BASIC, DIGEST, etc - **http-authentication-factory** and for protocols that require SASL - **sasl-authentication-factory**.
Example configuration of factories can look as follows:
...
Above example ties factories to **ManagementDomain**. Notice that for **local** realm mapper, SASL factory defines special mechanism **JBOSS-LOCAL-USER**. This mechanism maps local user, into local realm. Which in turn is mapped to SuperUser, given <<http-management-endpoint-securing-super-user>>.

=== Securing Management Interface

To secure management interface, configuration for those has to include reference to authentication factories. For HTTP based both http and SASL and for native, if it is defined - SASL.
Given previous examples, to secure both interfaces, following declaration could be used:

[source,xml,options="nowrap"]
----
<management-interfaces>
    <native-interface sasl-authentication-factory="management-sasl-authentication">
        <socket interface="management" port="jboss.management.native.port:9991}"/>
    </native-interface>
    <http-interface http-authentication-factory="management-http-authentication">
        <http-upgrade enabled="true" sasl-authentication-factory="management-sasl-authentication"/>
...
Notice, that HTTP interface require both SASL and HTTP authentication.

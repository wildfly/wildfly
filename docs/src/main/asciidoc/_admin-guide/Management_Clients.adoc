...
attributes are available to set to control the resource constraints for the HTTP management
interface. These attributes are:

* `backlog` - The number of connection requests allowed build up in a backlog attempting
to connect the open listener socket before being accepted by the server process (Default 50).
* `connection-high-water` - The maximum number of concurrent connection that the server will
...
When non-default values are set for these attributes the XML representation in the
standlone.xml will look like:
...
----
    <management-interfaces>
        <http-interface http-authentication-factory="management-http-authentication"
        backlog="75"
        no-request-timeout="60025"
        connection-high-water="125"
        connection-low-water="100">
            <http-upgrade
                enabled="true"
...
Or via the CLI this will look like:

[source,options="nowrap"]
----
[standalone@localhost:9990 /] /core-service=management/management-interface=http-interface:read-resource(include-defaults=true)
{
    "outcome" => "success",
    "result" => {

...
As the `constant-headers` attribute is set verification will be performed to verify that the HTTP headers specified are only making use of allowed characters as specified in the HTTP specification RFCs.  Additionally as they have special handling within the management interface overriding the headers `Connection` and `Date` is disallowed and attempts to set these will result in an error being reported.

To ensure the headers are set before the request starts to be sent to the client we intercept the request as it arrives for processing and set the configured headers before dispatching to the appropriate endpoint, this does mean if the specific endpoint also sets the same header it will likely replace any configured header.  This replacement however can not be relied upon as an endpoint could inspect if a header has already been set before deciding to set it's own.  In general this configuration is for setting headers the endpoints don't already set.

...
| ```encoding```
| Encoding used by the audit file. 
| String of ```UTF-8```, ```UTF-16BE```, ```UTF-16LE```, ```UTF-16```, ```US-ASCII``` or ```ISO-8859-1```
| ```UTF-8```
|===
...
==== Full UDP Definition with Infinite Reconnect Attempts, JSON Format and UTF-16 encoding.

Using the following command will generate a syslog audit logging resource that connects with
UDP, always attempts to send messages despite previous failures sending messages, records audit
events in a JSON format, uses UTF-16 file encoding and RFC5424 to describe the audit event.

...
[NOTE]
====
Beginning with WildFly 42, the `org.wildfly.extension.security.manager` extension and its `security-manager` subsystem are retained only as a legacy management model; starting a WildFly {wildflyVersion} standalone server with the extension in the configuration will result in a boot failure, unless the server is started in admin-only mode. It's completely valid for a standard WildFly {wildflyVersion} domain mode Domain Controller to have the `security-manager` subsystem in its `domain.xml` file, if the profiles that include the subsystem are used in server groups where all the servers are running WildFly 39 or earlier or the WildFly EE 10 variant of WildFly 40 or 41.
====

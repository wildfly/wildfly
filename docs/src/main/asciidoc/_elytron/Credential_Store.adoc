...
For credential stores backed by external vaults such as HashiCorp Vault, see <<Vault_Integration,Vault Integration>>.
...
Using a management operation the following command will generate and store a new key pair credential under the alias `example` using the ECDSA algorithm with 256 key size:

[source,options="nowrap"]
----
[standalone@localhost:9990 /] /subsystem=elytron/credential-store=mycredstore:generate-key-pair(alias=example, algorithm=EC, size=256)
{"outcome" => "success"}
----

NOTE: Algorithm and size parameters are optional parameters and if not specified default values will be used.

Key pair credentials can also be imported by specifying a private key in OpenSSH format:

[source,options="nowrap"]
----
[standalone@localhost:9990 /] /subsystem=elytron/credential-store=mycredstore:import-key-pair(alias=example, openssh-private-key="-----BEGIN OPENSSH PRIVATE KEY-----
                                                   
...
Key pair credentials can also be imported from a file containing a private key in OpenSSH format:

[source,options="nowrap"]
----
[standalone@localhost:9990 /] /subsystem=elytron/credential-store=mycredstore:import-key-pair(alias=example, openssh-private-key-location=/home/user/.ssh/id_rsa, passphrase=secret)
{"outcome" => "success"}
----

NOTE: The parameter passphrase is optional and can be empty if no passphrase was used to encrypt the key.

If the key is specified in PKCS format rather than OpenSSH format, both the private and public key must be defined and the PKCS private key must not be encrypted with a passphrase. Key pair credentials in PKCS format can be as well imported by providing the private key and public key value:

[source,options="nowrap"]
----
[standalone@localhost:9990 /] /subsystem=elytron/credential-store=mycredstore:import-key-pair(alias=example, private-key="-----BEGIN PRIVATE KEY-----
...
Key pair credentials in PCKS format can also be imported from files containing a private and public key:

[source,options="nowrap"]
----
[standalone@localhost:9990 /] /subsystem=elytron/credential-store=mycredstore:import-key-pair(alias=example, private-key-location=/home/user/.ssh/id_ecdsa_pkcs, public-key-location=/home/user/.ssh/id_ecdsa_pkcs.pub)
{"outcome" => "success"}
----

The public key can be then exported using the following command:

[source,options="nowrap"]
----
[standalone@localhost:9990 /] /subsystem=elytron/credential-store=mycredstore:export-key-pair-public-key(alias=example)
{
    "outcome" => "success",
    "result" => {"public-key" => "ecdsa-sha2-nistp256 AAAAE2VjZHNhLXNoYTItbmlzdHAyNTYAAAAIbmlzdHAyNTYAAABBBKeBzFRMF/Zq0YLmVBYK6mxAZE3DCcrpbD1e3ftKZoe/RtfmfUqewBqXTKQlUpmwPRnT2NUX3mMvq53BoqlZDBs="}
}
----

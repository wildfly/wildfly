...
The charset is UTF-8 and the string encoding is BASE64. The credential-store being referenced is called `mycredstore` and the alias of the key in that credential-store is `key`. The `key` secret key will be used to encrypt and decrypt the filesystem realm. The key-store being referenced is called `keystore` and the alias of the key pair in that key-store is `localhost`. The `localhost` key pair will be used to sign identity files and verify the pre-existing signatures to ensure that the filesystem realm has not been tampered with.

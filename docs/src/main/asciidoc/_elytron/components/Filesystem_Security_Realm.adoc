...
The charset is UTF-8 and the string encoding is BASE64. The credential-store being referenced is called `mycredstore` and the alias of the key in that credential-store is `key`. The `key` secret key will be used to encrypt and decrypt the filesystem realm

...
Beginning with WildFly 42, the `org.wildfly.extension.security.manager` extension and its `security-manager` subsystem are retained only as a legacy management model; it's completely valid for a standard WildFly {wildflyVersion} domain mode Domain Controller to have the `security-manager` subsystem in its `domain.xml` file, if the profiles that include the subsystem are used in server groups where all the servers are running WildFly 39 or earlier.

A standalone WildFly {wildflyVersion} server running in normal (non-admin-only) mode cannot use this extension or subsystem; the server will fail to start if the configuration contains it. The subsystem no longer grants or validates deployment permissions. Standard out-of-the-box configurations do not include the extension or subsystem.

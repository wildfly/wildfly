...
In this article, I'd like to show you how to setup WildFly {wildflyVersion} in domain
...
* Both master and slave will run WildFly {wildflyVersion}, and master will run as
...
mod_cluster module. The WildFly {wildflyVersion} on master and slave will form a
...
[[download-wildfly-{wildflyVersion}]]
== Download WildFly {wildflyVersion}

First we should download WildFly {wildflyVersion} from the website:
...
The version I downloaded is WildFly {wildflyVersion}.0.0.Final.

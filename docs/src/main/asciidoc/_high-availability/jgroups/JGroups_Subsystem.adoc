...
/socket-binding-group=standard-sockets/socket-binding=jgroups-diagnostics:add(multicast-address=224.0.75.75, multicast-port=7500)

# Enable diagnostics on 'tcp' stack
/subsystem=jgroups/stack=tcp/transport=TCP:write-attribute(name=diagnostics-socket-binding, value=jgroups-diagnostics)

# Enable diagnostics on 'udp' stack
...
[WARNING]
====
The diagnostics endpoint is unauthenticated; restrict access to trusted hosts via firewall rules and disable diagnostics support as soon as it is no longer being actively used.
====

To disable diagnostics support, simply undefine the `diagnostics-socket-binding` attribute on the transports and remove the socket binding.

[source,options="nowrap"]
----
# Disable diagnostics on 'tcp' stack
/subsystem=jgroups/stack=tcp/transport=TCP:undefine-attribute(name=diagnostics-socket-binding)

# Disable diagnostics on 'udp' stack
/subsystem=jgroups/stack=udp/transport=UDP:undefine-attribute(name=diagnostics-socket-binding)

# Remove jgroups-diagnostics socket binding
/socket-binding-group=standard-sockets/socket-binding=jgroups-diagnostics:remove
----

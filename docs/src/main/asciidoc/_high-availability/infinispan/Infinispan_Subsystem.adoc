...
A cache container can be injected directly into EE applications using the `@Resource` annotation, e.g.
...
segmented::
    ...
    Not supported for all cache stores.
    Default is false.
shared::
...
The JDBC store uses a database table per cache, with each row representing a single cache entry.
The store expects the table schema to contain specific columns, the details of which are described below.
By default, Infinispan will auto-create the table (based on configuration) if it does not already exist.
...
The `dialect` attribute is optional.
When undefined, Infinispan assumes standard SQL:2023 types appropriate for a given column.
The dialect should be specified when these default column types are not compatible with the target database and database-specific types need to be used instead.

====== Table configuration

A JDBC store requires a table configuration, defined as a child resource with the fixed path `table=string` (noting that `string` refers to the type of the indexable key).
If not explicitly configured, a default table configuration is created automatically.

e.g. To configure the JDBC store table:

[source,options="nowrap"]
----
/subsystem=infinispan/cache-container=foo/local-cache=bar/store=jdbc/table=string:add(prefix=my_cache)
----

The table resource supports the following attributes:

prefix::
    The prefix used to generate the database table name.
    Default is `ispn_entry`.
fetch-size::
    The number of rows fetched from the database at a time.
    Default is `100`.
create-on-start::
    Whether the table should be created when the cache starts, if it does not already exist.
    Default is `true`.
drop-on-stop::
    Whether the table should be dropped when the cache stops.
    Default is `false`.

====== Column configuration

Each table defines 4 columns.
All columns have sensible defaults and do not need to be explicitly configured unless custom column names or SQL types are required by the dialect.
Each column attribute accepts an object with `name` (the database table column name) and `type` (the SQL type) fields.

id-column::
    Stores the cache entry key in string form.
    Cache keys are converted to strings using an implementation of `org.infinispan.persistence.keymappers.TwoWayKey2StringMapper`, loaded dynamically (via `ServiceLoader`) from the module associated with the cache container.
    The default WildFly server configurations already supply implementations suitable for the associated use cases.
    Default name is `id`, default type is `VARCHAR`.
data-column::
    Stores the marshalled cache value and entry metadata as binary data.
    Cache values are marshalled using the user marshaller of the associated cache container.
    Default name is `datum`, default type is `BINARY`.
segment-column::
    Stores the consistent hash segment of the cache entry.
    This column may not be present if the cache store has segmentation disabled.
    This is an indexed column.
    Default name is `segment`, default type is `INTEGER`.
timestamp-column::
    Stores the cache entry version/timestamp.
    This is an indexed column.
    Default name is `version`, default type is `BIGINT`.

IMPORTANT: The `data-column` stores marshalled binary data, thus its SQL type must be a binary type.
Some databases require a database-specific type, e.g. `BYTEA` for PostgreSQL instead of `BINARY`.
Depending on the size, some databases may require using a BLOB type.

====== Complete JDBC store example

The following batch configures a distributed cache with a JDBC store persisting to a PostgreSQL database as a example:

[source,options="nowrap"]
----
batch
/subsystem=infinispan/cache-container=foo/distributed-cache=bar:add()
/subsystem=infinispan/cache-container=foo/distributed-cache=bar/store=jdbc:add(data-source=PostgresDS, dialect=POSTGRES)
/subsystem=infinispan/cache-container=foo/distributed-cache=bar/store=jdbc/table=string:add(prefix=my_cache)
/subsystem=infinispan/cache-container=foo/distributed-cache=bar/store=jdbc/table=string:write-attribute(name=id-column, value={name=id, type=VARCHAR})
/subsystem=infinispan/cache-container=foo/distributed-cache=bar/store=jdbc/table=string:write-attribute(name=data-column, value={name=datum, type=BYTEA})
/subsystem=infinispan/cache-container=foo/distributed-cache=bar/store=jdbc/table=string:write-attribute(name=segment-column, value={name=segment, type=INTEGER})
/subsystem=infinispan/cache-container=foo/distributed-cache=bar/store=jdbc/table=string:write-attribute(name=timestamp-column, value={name=version, type=BIGINT})
run-batch
----

The resulting XML configuration:

[source,xml,options="nowrap"]
----
<distributed-cache name="bar">
    <jdbc-store data-source="PostgresDS" dialect="POSTGRES">
...
For a complete list of JDBC store attributes, refer to the link:feature-pack/doc/reference/index.html[WildFly management model documentation^].
...
==== Injecting a cache into EE applications

A cache can be injected directly into EE applications using the `@Resource` annotation, e.g.
...
A HotRod client can be injected directly into EE applications using the `@Resource` annotation.

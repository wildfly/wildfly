...
Cache entries are serialized and stored as binary data – the store does not map cache keys and values directly to columns.
...
It should be specified when the default column types (e.g. `BINARY`, see defaults below) are not compatible with the target database and database-specific types need to be used instead.
...
id-column::
    Stores the cache entry key.
    Default name is `id`, default type is `VARCHAR`.
data-column::
    Stores the serialized cache entry value as binary data.
...
IMPORTANT: The `data-column` stores serialized binary data, not the raw cache value,
thus its SQL type must be a binary type.

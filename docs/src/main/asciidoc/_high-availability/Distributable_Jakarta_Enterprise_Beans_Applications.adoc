...
is not guaranteed to survive beyond the lifespan of the EJB container. And, as with standard web applications, there *is* a way to
...
However, in the case of EJB applications, whether a bean is distributable or not is determined on a per-bean basis, not
an application-wide basis as in the case of web applications through the use of the <distributable/> element.

A stateful session bean within an EJB application indicates its intention to be distributable by using a passivation-capable cache factory
to store its session states. Cache factories were discussed in Section 7.6.5 of link:Admin_Guide{outfilesuffix}[Wildfly Admin Guide].
Additionally, the EJB application needs to be deployed into a server which uses an HA (High Availability) server profile, such as standalone-ha.xml
or standalone-full-ha.xml.

NOTE: More information on passivation-capable beans can be found in the EJB 3.2 specification document, Section 4.6.5.
...
[[distributable-ejb-default-configurations]]
=== distributable-ejb subsystem default configurations

The default configurations for the distributable-ejb subsystem are as follows:

HA server profiles (i.e. standalone-ha.xml or standalone-full-ha.xml):
[source,xml]
----
...
non-HA server profiles (i.e. standalone.xml or standalone-full.xml) :
[source,xml]
----
<subsystem xmlns="urn:jboss:domain:distributable-ejb:1.0" default-bean-management="default">

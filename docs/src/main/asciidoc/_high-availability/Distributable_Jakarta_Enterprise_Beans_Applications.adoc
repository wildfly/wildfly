...
However, in the case of Jakarta Enterprise Beans applications, whether or not a bean is distributable is determined globally or on a per-bean basis, rather than on
...
Cache factories were discussed in the link:Admin_Guide{outfilesuffix}#distributable-cache[Jakarta Enterprise Beans section of the Wildfly Admin Guide].

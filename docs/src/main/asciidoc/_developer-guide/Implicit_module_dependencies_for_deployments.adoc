...
need access to classes from the jakarta.ejb.* package and other EE
...
contains all the EE API classes. For an application to be able to
...
deployment. Let's take an example - Consider (again) a Jakarta Enterprise Beans 3 deployment
...
then add an implicit dependency on the EE API module, so that all
the EE API classes are visible to the deployment.

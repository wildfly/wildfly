...
https://jakarta.ee/specifications/restful-ws/2.1/apidocs/javax/ws/rs/client/Client.html[`javax.ws.rs.client.Client`] and `javax.ws.rs.core.Response`. See the `resteasy-jaxrs-client` quickstart for an example of an external Jakarta RESTful Web Services RESTEasy client that interacts with a Jakarta RESTful Web Services.

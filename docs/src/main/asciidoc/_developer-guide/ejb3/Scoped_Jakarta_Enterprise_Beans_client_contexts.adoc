...
Now, we have seen in this <<EJB_invocations_from_a_remote_client_using_JNDI, other chapter>>
...
https://redhat.atlassian.net/browse/EJBCLIENT-34 feature request. The
...
functionality through https://redhat.atlassian.net/browse/EJBCLIENT-34. We
want the user applications to have more control over which EJB receiver
gets used for a specific invocation.

Before we introduced https://redhat.atlassian.net/browse/EJBCLIENT-34
feature, the EJB client context was typically scoped to the client
application. As part of https://redhat.atlassian.net/browse/EJBCLIENT-34 we

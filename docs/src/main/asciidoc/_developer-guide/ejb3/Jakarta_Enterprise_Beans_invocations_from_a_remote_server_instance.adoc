...
<<EJB_invocations_from_a_remote_client_using_JNDI, from a remote standalone client>>
...
other components (like EJB) too. Syntax used in Greeter bean lookup is
explained <<EJB_invocations_from_a_remote_client_using_JNDI, here>>.
...
public void invokeOnBean() {
        try {
            ...
            // Lookup the Greeter bean using the ejb: namespace syntax which is explained in the link above

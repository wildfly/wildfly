...
main directory is for Faces 2.0 and the one located in the 1.2 directory
is for Faces 1.2. If there was only one module available, we could simply
create a `MANIFEST.MF` file and added the module dependency. But in this
case, we want to use the Faces 1.2 version and not the 2.0 version in
...
`javax.faces.api` for the Faces 1.2 module. We also add the dependency for
the Faces 1.2 module in the sub-deployment section for the WAR and exclude
the module for Faces 2.0.
...
want to use the Faces 1.2 version and not the Faces 2.0 version in main, so
...
subdeployment and exclude the Faces 2.0 module. The file should now look
like this:

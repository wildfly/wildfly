...
you are deploying an EE application, a dependency on the EE
API's will be added to your module automatically. Similarly, if your
module contains a beans.xml file a dependency on
https://weld.cdi-spec.org/[Weld] will be added automatically, along
...
module automatically by the container, including the Jakarta specification api's.
...
The EE Platform specification says that portable applications should not
...
https://github.com/wildfly/wildfly-core/blob/main/server/src/main/resources/schema/jboss-deployment-structure-1_3.xsd[jboss-deployment-structure-1_3.xsd]

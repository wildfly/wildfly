...
EE 7, which adapts well known Java SE concurrency utilities to the
EE application environment specifics. The EE application server is
...
the default instance mandated by the EE Platform specification, and such
...
As mandated by the EE Platform specification, the default Context Service
...
Thread Factory ( `java.util.concurrent.ThreadFactory`) adapted to the EE platform specifics.

Managed Thread Factory instances are managed by the application server,
thus EE applications are forbidden to invoke any lifecycle related
method.
...
As mandated by the EE Platform specification, the default Managed Thread
...
`java.util.concurrent.ExecutorService`) adapted to the EE platform
requirements.

Managed Executor Service instances are managed by the application
server, thus EE applications are forbidden to invoke any lifecycle
related method.
...
As mandated by the EE Platform specification, the default Managed Executor
...
EE applications to schedule tasks for asynchronous execution. It is
...
application server, thus EE applications are forbidden to invoke
...
As mandated by the EE Platform specification, the default Managed Scheduled

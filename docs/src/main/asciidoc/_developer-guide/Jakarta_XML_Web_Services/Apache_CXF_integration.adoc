...
WildFly is much more than a servlet container; it actually provides
users with a fully compliant target platform for EE applications.
...
compliant platforms. Being an EE Platform implementation, WildFly already comes
...
anymore). This is really evident in an EE environment (hence when

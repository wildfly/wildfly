...
|wildfly.jpa.emcache |Set to true to enable caching of entity manager instances (currently only for transaction scoped within JTA transactions), defaults to false.  Note that before an entity manager is put in the cache, EntityManager.clear() is called.  Each persistence unit has its own entity manager cache.

|wildfly.jpa.emcache.size |Set to the cache size used with wildfly.jpa.emcache, defaults to 0 (for unbounded cache), specify size greater than zero for a bounded cache.

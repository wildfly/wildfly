...
https://javaee.github.io/javaee-spec/javadocs/javax/persistence/package-summary.html[Javadoc
...
http://infinispan.org/docs/9.2.x/user_guide/user_guide.html#jpa_hibernate_2l_cache contains
...
Hibernate ORM persistence provider - this will be automatically
enabled. +
...
to use the version 5.4.0.Alpha1 instead of the provided
Search module; in this case you'll have to download and add the custom modules
...
Applied Transformations to application classes compiled with Hibernate 5.1: ::
...
*  In user type classes, when calling hibernate.orm methods, cast SessionImplementor to SharedSessionContractImplementor.
...
|Hibernate51CompatibilityTransformer=false| if true, the transformer will transform ORM 5.1.x native calls to, ORM 5.3 native calls, in every application deployment.

|Hibernate51CompatibilityTransformer.disableAmbiguousChanges=false| disable transformation of org.hibernate.Query.setFirstResult(int) + org.hibernate.Query.setMaxResults(int), since these methods are in both Hibernate ORM 5.1.x + 5.3.x.

|Hibernate51CompatibilityTransformer.showTransformedClassFolder| specifies the folder name to create bytecode level description of applications run through the transformer.  If the transformer actually makes a change to your application, a (boolean) class variable will be added "$_org_jboss_as_hibernate_Hibernate51CompatibilityTransformer_transformed_$", that ensures that the application is only transformed once.
| 
----
----
...
if *new_generator_mappings* is *true*:
...
if *new_generator_mappings* is *false*:

...
The EE platform specification defines the following JNDI contexts:
...
For EE applications the recommended way is to use a
...
Standard EE applications may use the standard JNDI API, included
...
mandated by the EE Platform Specification).
...
out of an EE application context, using the standard JNDI API may
...
For EE applications the recommended way to look up a JNDI entry is
...
EE applications may use, without any additional configuration

...
public class MessagingTransformerRegistration implements ExtensionTransformerRegistration {
    ...
    private static void registerTransformers_WF_36(ResourceTransformationDescriptionBuilder subsystem) {
        subsystem.addChildResource(LIVE_ONLY_PATH).getAttributeBuilder().addRejectCheck(DEFINED, ScaleDownAttributes.SCALE_DOWN_COMMIT_INTERVAL);
        subsystem.addChildResource(REPLICATION_SECONDARY_PATH).getAttributeBuilder().addRejectCheck(DEFINED, ScaleDownAttributes.SCALE_DOWN_COMMIT_INTERVAL);
        subsystem.addChildResource(SHARED_STORE_SECONDARY_PATH).getAttributeBuilder().addRejectCheck(DEFINED, ScaleDownAttributes.SCALE_DOWN_COMMIT_INTERVAL);
    }

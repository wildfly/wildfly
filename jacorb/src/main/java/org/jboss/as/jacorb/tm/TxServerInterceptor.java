...
public class TxServerInterceptor extends LocalObject implements ServerRequestInterceptor {
     ...
     */
    static void init(final int slotId, final Codec codec, final org.omg.PortableInterceptor.Current piCurrent) {
    ...
    public static Transaction getCurrentTransaction() {
        ...
        if (piCurrent != null) {
            ...
            try {
                final Any any = piCurrent.get_slot(slotId);
                if (any.type().kind().value() != TCKind._tk_null) {
                    // Yes, there is a TPC: add the foreign transaction marker
                    tx = ForeignTransaction.instance;
                }
            } catch (InvalidSlot e) {
                throw new RuntimeException("Exception getting slot in TxServerInterceptor: " + e);
            }

...
class MavenUtil {
    ...
    private static volatile MavenUtil INSTANCE;
    ...
    static MavenUtil getInstance(boolean useEapRepository) {
        if (INSTANCE == null) {
            synchronized (AETHER_API_NAME) {
                if (INSTANCE == null) {
                    MavenUtil util = new MavenUtil(newRepositorySystem(), createRemoteRepositories(useEapRepository));
                    INSTANCE = util;
                }
            }
        }

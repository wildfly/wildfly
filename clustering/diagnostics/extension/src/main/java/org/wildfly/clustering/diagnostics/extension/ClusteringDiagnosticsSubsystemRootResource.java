 ...
 */
public class ClusteringDiagnosticsSubsystemRootResource implements Resource {

    private final Resource delegate;
    private volatile ServiceRegistry registry;

    public ClusteringDiagnosticsSubsystemRootResource() {
        this(Factory.create());
    }

    public ClusteringDiagnosticsSubsystemRootResource(final Resource delegate) {
    ...
    public Set<String> getChildrenNames(String childType) {
        if (ModelKeys.CLUSTER.equals(childType)) {
            return ClusteringDiagnosticsSubsystemHelper.getChannelNames(this.registry);
        } else {
    ...
    public Set<ResourceEntry> getChildren(String childType) {
        if (ModelKeys.CLUSTER.equals(childType)) {
            ...
            for (ServiceName serviceName : ClusteringDiagnosticsSubsystemHelper.getChannelServiceNames(this.registry)) {
                // build up a set of ResourceEntry descriptions
                String channel = ClusteringDiagnosticsSubsystemHelper.getChannelNameFromChannelServiceName(serviceName);
    ...
    public Resource clone() {
        ClusteringDiagnosticsSubsystemRootResource clone = new ClusteringDiagnosticsSubsystemRootResource(delegate.clone());
    ...
    private boolean hasChannel(PathElement element) {
        ...
        ServiceName channelName = ClusteringDiagnosticsSubsystemHelper.CHANNEL_PARENT.append(element.getValue());
        if (!ClusteringDiagnosticsSubsystemHelper.isValidChannelServiceName(channelName)) {
            return false;
        }
        ServiceController<?> controller = registry.getService(channelName);
        return (controller != null) && ClusteringDiagnosticsSubsystemHelper.serviceIsUp(this.registry, channelName);
    }

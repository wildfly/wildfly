 ...
 * @author Paul Ferraro
 */
public class ProtocolConfigurationResourceDefinitionRegistrar<P extends Protocol, C extends ProtocolConfiguration<P>> extends ProtocolChildResourceDefinitionRegistrar {

    interface ProtocolConfigurationResourceDescriptorConfigurator<P extends Protocol, C extends ProtocolConfiguration<P>> extends ProtocolChildResourceDescriptorConfigurator, ResourceServiceConfigurator, ResourceModelResolver<ServiceDependency<C>> {
        RuntimeCapability<Void> getCapability();

        @Override
        default ResourceDescriptor.Builder apply(ResourceDescriptor.Builder builder) {
            return ProtocolChildResourceDescriptorConfigurator.super.apply(builder).addCapability(this.getCapability())
                    .withRuntimeHandler(ResourceOperationRuntimeHandler.combine(ResourceOperationRuntimeHandler.configureService(this), ResourceOperationRuntimeHandler.restartParent(this.getParentRuntimeHandler())))
                    ;
        }

        @Override
        default ResourceServiceInstaller configure(OperationContext context, ModelNode model) throws OperationFailedException {
            return CapabilityServiceInstaller.builder(this.getCapability(), this.resolve(context, model)).blocking().build();
        }

        ResourceOperationRuntimeHandler getParentRuntimeHandler();

        default ResourceModelResolver<ServiceDependency<ProtocolConfiguration<P>>> getProtocolConfigurationResolver() {
            return new ResourceModelResolver<>() {
                ...
                public ServiceDependency<ProtocolConfiguration<P>> resolve(OperationContext context, ModelNode model) throws OperationFailedException {
                    ...
                    ServiceDependency<Module> module = ProtocolChildResourceDescription.MODULE.resolve(context, model);
                    Map<String, String> properties = ProtocolChildResourceDescription.PROPERTIES.resolve(context, model);
                    Boolean statisticsEnabled = ProtocolChildResourceDescription.STATISTICS_ENABLED.resolve(context, model);
                            ...
                            return new ProtocolConfiguration<>() {
                                ...
                                public P createProtocol(ChannelFactoryConfiguration configuration) {
                                    try {
                                        ...
                                        boolean nativeProtocol = module.get().getName().equals(ProtocolChildResourceDescription.MODULE.getDefaultValue().asString()) && !name.startsWith(Global.PREFIX);
    ...
    ProtocolConfigurationResourceDefinitionRegistrar(ProtocolConfigurationResourceDescriptorConfigurator<P, C> configurator) {
        super(configurator);
    }

...
public enum StackOperation implements RuntimeOperation<ChannelFactory> {
    ...
    StackOperation(String name, ModelType replyType) {
        this.definition = new SimpleOperationDefinitionBuilder(name, JGroupsSubsystemResourceDefinitionRegistrar.RESOLVER.createChildResolver(StackResourceDefinitionRegistrar.WILDCARD_PATH)).setReplyType(replyType).setReadOnly().build();
    }

...
public enum StackOperation implements RuntimeOperation<ChannelFactory> {
    ...
    StackOperation(String name, ModelType replyType) {
        this.definition = new SimpleOperationDefinitionBuilder(name, JGroupsSubsystemResourceDefinitionRegistrar.RESOLVER.createChildResolver(StackResourceDescription.INSTANCE.getPathElement())).setReplyType(replyType).setReadOnly().build();
    }

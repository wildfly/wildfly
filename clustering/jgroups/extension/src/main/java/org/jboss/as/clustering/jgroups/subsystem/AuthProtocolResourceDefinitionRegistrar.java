...
public class AuthProtocolResourceDefinitionRegistrar extends AbstractProtocolResourceDefinitionRegistrar<AUTH> {
    ...
    AuthProtocolResourceDefinitionRegistrar(String name, ResourceOperationRuntimeHandler parentRuntimeHandler) {
        super(new ProtocolResourceRegistration<>() {
            ...
            public ServiceDependency<ProtocolConfiguration<AUTH>> resolve(OperationContext context, ModelNode model) throws OperationFailedException {
                return ProtocolResourceRegistration.super.resolve(context, model).combine(TOKEN.resolve(context, model), new BiFunction<>() {
                    @Override

...
public class AuthProtocolResourceDefinitionRegistrar extends AbstractProtocolResourceDefinitionRegistrar<AUTH> {
    ...
    private static final ResourceCapabilityReference<AuthToken> TOKEN = ResourceCapabilityReference.builder(AbstractProtocolResourceDefinitionRegistrar.CAPABILITY, AuthTokenResourceDescription.SERVICE_DESCRIPTOR).build();

    AuthProtocolResourceDefinitionRegistrar(AuthProtocolResourceDescription description, ResourceOperationRuntimeHandler parentRuntimeHandler) {
        super(new ProtocolResourceDescriptorConfigurator<>() {
            ...
            public ServiceDependency<ProtocolConfiguration<AUTH>> resolve(OperationContext context, ModelNode model) throws OperationFailedException {
                return ProtocolResourceDescriptorConfigurator.super.resolve(context, model).combine(TOKEN.resolve(context, model), new BiFunction<>() {
                    @Override

...
public class ChannelResourceDefinitionRegistrar extends AbstractChannelResourceDefinitionRegistrar<ChannelConfiguration> {
    static final RuntimeCapability<Void> CAPABILITY = RuntimeCapability.Builder.of(ChannelConfiguration.SERVICE_DESCRIPTOR).build();
    ...
    private ChannelResourceDefinitionRegistrar(ServiceValueExecutorRegistry<JChannel> channelRegistry) {
        super(new ChannelResourceDescriptorConfigurator<>() {
            @Override
            public ResourceDescription getResourceDescription() {
                return ChannelResourceDescription.INSTANCE;
            }
            ...
            public ResourceDescriptor.Builder apply(ResourceDescriptor.Builder builder) {
                ...
                return ChannelResourceDescriptorConfigurator.super.apply(builder).addCapabilities(List.of(commandDispatcherFactory, legacyCommandDispatcherFactory, group, legacyGroup));
            }
            ...
            public ResourceModelResolver<ServiceDependency<ChannelConfiguration>> getChannelConfigurationResolver() {
                return new ResourceModelResolver<>() {
                    ...
                    public ServiceDependency<ChannelConfiguration> resolve(OperationContext context, ModelNode model) throws OperationFailedException {
                        String name = context.getCurrentAddressValue();
                        String clusterName = ChannelResourceDescription.CLUSTER.resolveModelAttribute(context, model).asString(name);
                        boolean statisticsEnabled = ChannelResourceDescription.STATISTICS_ENABLED.resolve(context, model);
                        return ChannelResourceDescription.STACK.resolve(context, model).combine(ChannelResourceDescription.MODULE.resolve(context, model), new BiFunction<>() {
                            @Override
            ...
            public ResourceModelResolver<PathAddress> getStackAddressResolver() {
                return new ResourceModelResolver<>() {
                    ...
                    public PathAddress resolve(OperationContext context, ModelNode model) throws OperationFailedException {
                        String stack = ChannelResourceDescription.STACK.resolveModelAttribute(context, model).asString();
                        return context.getCurrentAddress().getParent().append(StackResourceDescription.pathElement(stack));
                    }
                };
            }
                    ...
                    public OperationStepHandler apply(OperationStepHandler handler) {
                        return new OperationStepHandler() {
                            ...
                            public void execute(OperationContext context, ModelNode operation) throws OperationFailedException {
                                ...
                                if (!operation.hasDefined(ChannelResourceDescription.STACK.getName())) {
                                    ...
                                    if (subsystemModel.hasDefined(JGroupsSubsystemResourceDescription.DEFAULT_CHANNEL.getName())) {
                                        String defaultChannel = subsystemModel.get(JGroupsSubsystemResourceDescription.DEFAULT_CHANNEL.getName()).asString();
                                        if (!context.getCurrentAddressValue().equals(defaultChannel)) {
                                            PathElement defaultChannelPath = ChannelResourceDescription.pathElement(defaultChannel);
                                            ...
                                            String defaultStack = channelModel.get(ChannelResourceDescription.STACK.getName()).asString();
                                            operation.get(ChannelResourceDescription.STACK.getName()).set(defaultStack);
                                        }
                                    }
                                }

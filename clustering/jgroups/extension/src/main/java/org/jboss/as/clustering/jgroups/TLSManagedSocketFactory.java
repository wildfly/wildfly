 ...
 * client/server {@link SSLContext}s provided by Elytron.
 *
...
public class TLSManagedSocketFactory extends ManagedSocketFactory {
    ...
    public Socket createSocket(String name) throws IOException {
        ...
        Closeable socketRegistration = this.manager.getNamedRegistry().registerSocket(name, socket);
    ...
    public ServerSocket createServerSocket(String name) throws IOException {
        ...
        Closeable registeredSocket = this.manager.getNamedRegistry().registerSocket(name, serverSocket);

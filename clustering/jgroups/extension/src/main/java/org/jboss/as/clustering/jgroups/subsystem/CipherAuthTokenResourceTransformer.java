...
public class CipherAuthTokenResourceTransformer extends AuthTokenResourceTransformer {
    ...
    CipherAuthTokenResourceTransformer(ResourceTransformationDescriptionBuilder parent) {
        super(parent.addChildResource(CipherAuthTokenResourceDefinitionRegistrar.PATH));
    }
    ...
    public void accept(ModelVersion version) {
        if (JGroupsSubsystemModel.VERSION_8_0_0.requiresTransformation(version)) {
            this.builder.getAttributeBuilder()
                    .addRejectCheck(CredentialReference.REJECT_CREDENTIAL_REFERENCE_WITH_BOTH_STORE_AND_CLEAR_TEXT, CipherAuthTokenResourceDefinitionRegistrar.KEY_CREDENTIAL.getName())
                    .end();
        }

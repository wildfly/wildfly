...
public class CipherAuthTokenResourceDefinitionRegistrar extends AuthTokenResourceDefinitionRegistrar<CipherAuthToken> {
    ...
    static Stream<AttributeDefinition> attributes() {
        return Stream.concat(AuthTokenResourceDefinitionRegistrar.attributes(), Stream.concat(Stream.of(KEY_CREDENTIAL, KEY_STORE), ResourceDescriptor.stream(EnumSet.allOf(Attribute.class))));
    }

    CipherAuthTokenResourceDefinitionRegistrar() {
        super(new TokenResourceRegistration() {
            @Override
            ...
            @Override
            public ResourceDescriptor.Builder apply(ResourceDescriptor.Builder descriptor) {
                return descriptor.provideAttributes(EnumSet.allOf(Attribute.class))

...
public enum JGroupsSubsystemSchema implements SubsystemResourceXMLSchema<JGroupsSubsystemSchema> {
    ...
    public SubsystemResourceRegistrationXMLElement getSubsystemXMLElement() {
        ...
        if (this.since(VERSION_3_0)) {
            ...
            builder.withContent(this.factory.sequence().addElement(this.stackElement()).build());
        }
        if (!this.since(VERSION_9_0)) {
    ...
    private ResourceRegistrationXMLElement channelElement() {
        NamedResourceRegistrationXMLElement.Builder builder = this.factory.namedElement(JGroupsResourceRegistration.CHANNEL).addAttribute(ChannelResourceDefinitionRegistrar.MODULE);
    ...
    private ResourceRegistrationXMLElement stackElement() {
        ...
        if (this.since(VERSION_2_0)) {
            contentBuilder.addElement(this.relayElement()).withCardinality(XMLCardinality.Single.OPTIONAL);
        }
        return builder.withContent(contentBuilder.build()).build();
    }

    private ResourceXMLChoice transportChoice() {
        ResourceRegistrationXMLChoice.Builder builder = this.factory.choice(this.transportBuilder(StackResourceDefinitionRegistrar.Component.TRANSPORT).build());
    ...
    private NamedResourceRegistrationXMLElement.Builder transportBuilder(ResourceRegistration registration) {
        NamedResourceRegistrationXMLElement.Builder builder = this.protocolChildElementBuilder(registration).withOperationKey(PathElement.pathElement(registration.getPathElement().getKey()));
        ...
        if (!this.since(VERSION_6_0)) {
            for (String localName : List.of("internal-thread-pool", "oob-thread-pool", "timer-thread-pool")) {
                contentBuilder.addElement(XMLElement.ignore(this.factory.resolve(localName), XMLCardinality.Single.OPTIONAL));
            }
        }
    ...
    private ResourceXMLChoice protocolChoice() {
        ResourceRegistrationXMLChoice.Builder builder = this.factory.choice(this.protocolElementBuilder(StackResourceDefinitionRegistrar.Component.PROTOCOL)
        ...
        if (this.since(VERSION_5_0)) {
            for (AuthProtocolResourceDefinitionRegistrar.Protocol protocol : EnumSet.allOf(AuthProtocolResourceDefinitionRegistrar.Protocol.class)) {
                ...
                for (AuthTokenResourceDefinitionRegistrar.Token token : EnumSet.allOf(AuthTokenResourceDefinitionRegistrar.Token.class)) {
                    ...
                    switch (token) {
                        case DIGEST:
                            ...
                            break;
                        case CIPHER:
                            tokenBuilder.addAttributes(EnumSet.allOf(CipherAuthTokenResourceDefinitionRegistrar.Attribute.class).stream().map(AttributeDefinitionProvider::get).toList());
                ...
                builder.addElement(protocolBuilder.withContent(this.protocolContentBuilder().addChoice(tokens.build()).build()).build());
            }
    ...
    private NamedResourceRegistrationXMLElement.Builder protocolChildElementBuilder(ResourceRegistration registration) {
        NamedResourceRegistrationXMLElement.Builder builder = this.factory.namedElement(registration).addAttribute(ProtocolChildResourceDefinitionRegistrar.PROPERTIES);
        if (this.since(VERSION_3_0)) {
    ...
    private ResourceRegistrationXMLElement relayElement() {
        ...
        return builder.withContent(this.factory.sequence().addElement(this.remoteSiteElement()).build()).build();
    }

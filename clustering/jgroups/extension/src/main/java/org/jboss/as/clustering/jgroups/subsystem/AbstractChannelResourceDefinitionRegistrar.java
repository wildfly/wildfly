 ...
 */
public abstract class AbstractChannelResourceDefinitionRegistrar<C extends ChannelConfiguration> implements ChildResourceDefinitionRegistrar, ResourceServiceConfigurator, ResourceOperationRuntimeHandler {

    static final RuntimeCapability<Void> CHANNEL = RuntimeCapability.Builder.of(JGroupsServiceDescriptor.CHANNEL).setAllowMultipleRegistrations(true).build();
    static final RuntimeCapability<Void> CHANNEL_CONFIGURATION = RuntimeCapability.Builder.of(ChannelConfiguration.SERVICE_DESCRIPTOR).setAllowMultipleRegistrations(true).build();
    static final RuntimeCapability<Void> CHANNEL_FACTORY = RuntimeCapability.Builder.of(ForkChannelFactory.SERVICE_DESCRIPTOR).setAllowMultipleRegistrations(true).build();

    private final ChannelResourceRegistration<C> registration;
    private final ServiceValueExecutorRegistry<JChannel> channelRegistry;

    interface ChannelResourceRegistration<C> extends ResourceRegistration, UnaryOperator<ResourceDescriptor.Builder>, ResourceServiceConfigurator {

        default UnaryOperator<OperationStepHandler> getAddOperationTransformer() {
    ...
    AbstractChannelResourceDefinitionRegistrar(ChannelResourceRegistration<C> registration, ServiceValueExecutorRegistry<JChannel> channelRegistry) {
    ...
    public ManagementResourceRegistration register(ManagementResourceRegistration parent, ManagementResourceRegistrationContext context) {
        UnaryOperator<OperationStepHandler> addOperationTransformer = this.registration.getAddOperationTransformer();

        ResourceOperationRuntimeHandler runtimeResourceRegistrationHandler = this;
        ResourceDescriptionResolver resolver = JGroupsSubsystemResourceDefinitionRegistrar.RESOLVER.createChildResolver(this.registration.getPathElement());
        ResourceDescriptor descriptor = this.registration.apply(ResourceDescriptor.builder(resolver))
                .addCapabilities(List.of(CHANNEL, CHANNEL_CONFIGURATION, CHANNEL_FACTORY))
                            ...
                            public void execute(OperationContext context, ModelNode operation) throws OperationFailedException {
                                ...
                                if (context.isDefaultRequiresRuntime()) {
                                    ...
                                    context.addStep(new OperationStepHandler() {
                                        @Override
                                        public void execute(OperationContext context, ModelNode operation) throws OperationFailedException {
                                            runtimeResourceRegistrationHandler.addRuntime(context, resource);
                                        }
                    ...
                    public OperationStepHandler apply(OperationStepHandler handler) {
                        return new OperationStepHandler() {
                            ...
                            public void execute(OperationContext context, ModelNode operation) throws OperationFailedException {
                                ...
                                if (context.isDefaultRequiresRuntime()) {
                                    runtimeResourceRegistrationHandler.removeRuntime(context, resource);
                                }
                            }
                        };
                    }
    ...
    public ResourceServiceInstaller configure(OperationContext context, ModelNode model) throws OperationFailedException {
        ...
        // Create installer for service providing a channel configuration
        installers.add(CapabilityServiceInstaller.builder(CHANNEL_CONFIGURATION, this.registration.getChannelConfigurationResolver().resolve(context, model)).build());

        installers.add(this.registration.configure(context, model));
        ...
        installers.add(new BinderServiceInstaller(JGroupsBindingFactory.CHANNEL.apply(name), CHANNEL.getCapabilityServiceName(address)));
        installers.add(new BinderServiceInstaller(JGroupsBindingFactory.CHANNEL_FACTORY.apply(name), CHANNEL_FACTORY.getCapabilityServiceName(address)));
    ...
    public void addRuntime(OperationContext context, ModelNode model) throws OperationFailedException {
        ...
        PathAddress stackAddress = this.registration.getStackAddressResolver().resolve(context, resource);
        Resource stackResource = address.equals(stackAddress) ? resource : context.readResourceFromRoot(stackAddress);
        List<PathElement> protocolTypes = List.of(TransportResourceDefinitionRegistrar.WILDCARD_PATH, ProtocolResourceDefinitionRegistrar.WILDCARD_PATH, RelayResourceDefinitionRegistrar.WILDCARD_PATH);
        if (protocolTypes.stream().anyMatch(protocolType -> stackResource.hasChildren(protocolType.getKey()))) {
            ...
            if (resource != stackResource) {
                ...
                OverrideDescriptionProvider provider = new OverrideDescriptionProvider() {
                    ...
                    public Map<String, ModelNode> getChildTypeOverrideDescriptions(Locale locale) {
                        String description = JGroupsSubsystemResourceDefinitionRegistrar.RESOLVER.getChildTypeDescription(ProtocolResourceDefinitionRegistrar.WILDCARD_PATH.getKey(), locale, JGroupsSubsystemResourceDefinitionRegistrar.RESOLVER.getResourceBundle(locale));
                        ModelNode result = new ModelNode();
                        result.get(ModelDescriptionConstants.DESCRIPTION).set(description);
                        return Collections.singletonMap(ProtocolResourceDefinitionRegistrar.WILDCARD_PATH.getKey(), result);
                    }
                };
            ...
            for (PathElement path : protocolTypes) {
                for (Resource.ResourceEntry protocolResource : stackResource.getChildren(path.getKey())) {
                    ...
                    resource.registerChild(AbstractProtocolResourceDefinitionRegistrar.pathElement(protocolName), PlaceholderResource.INSTANCE);
                }
            }
        }
    }
    ...
    public void removeRuntime(OperationContext context, ModelNode model) throws OperationFailedException {
        PathAddress address = context.getCurrentAddress();
        PathAddress stackAddress = this.registration.getStackAddressResolver().resolve(context, model);
        ManagementResourceRegistration registration = context.getResourceRegistrationForUpdate();
    ...
    private ManagementResourceRegistration register(ManagementResourceRegistration parent, String protocolName, Class<? extends Protocol> protocolClass) {
        ...
        PathElement protocolPath = AbstractProtocolResourceDefinitionRegistrar.pathElement(protocolName);
        ManagementResourceRegistration registration = parent.getSubModel(PathAddress.pathAddress(protocolPath));
        if (registration == null) {
            ...
            ResourceDefinition definition = ResourceDefinition.builder(ResourceRegistration.of(AbstractProtocolResourceDefinitionRegistrar.pathElement(protocolName)), resolver).asRuntime().build();

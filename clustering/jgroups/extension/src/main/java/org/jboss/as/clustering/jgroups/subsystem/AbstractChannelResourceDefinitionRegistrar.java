...
public abstract class AbstractChannelResourceDefinitionRegistrar<C extends ChannelConfiguration> implements ChildResourceDefinitionRegistrar, ResourceServiceConfigurator, ResourceOperationRuntimeHandler {
    ...
    interface ChannelResourceDescriptorConfigurator<C> extends UnaryOperator<ResourceDescriptor.Builder>, ResourceServiceConfigurator {
        ...
        default ResourceDescriptor.Builder apply(ResourceDescriptor.Builder builder) {
            return builder.addAttributes(this.getResourceDescription().getAttributes().collect(Collectors.toList())).addCapabilities(List.of(this.getCapability(), CHANNEL, CHANNEL_FACTORY));
        }
        ...
        default ResourceServiceInstaller configure(OperationContext context, ModelNode model) throws OperationFailedException {
            ...
            ResourceServiceInstaller channelFactoryInstaller = CapabilityServiceInstaller.builder(CHANNEL_FACTORY, org.jboss.as.clustering.jgroups.ForkChannelFactory::new, configuration)
            ...
            ResourceServiceInstaller channelConfigurationInstaller = CapabilityServiceInstaller.builder(this.getCapability(), this.getChannelConfigurationResolver().resolve(context, model)).build();
            return ResourceServiceInstaller.combine(channelFactoryInstaller, channelConfigurationInstaller);
        }
    ...
    private final ChannelResourceDescriptorConfigurator<C> configurator;
    private final ServiceValueExecutorRegistry<JChannel> channelRegistry;

    AbstractChannelResourceDefinitionRegistrar(ChannelResourceDescriptorConfigurator<C> configurator, ServiceValueExecutorRegistry<JChannel> channelRegistry) {
    ...
    public ManagementResourceRegistration register(ManagementResourceRegistration parent, ManagementResourceRegistrationContext context) {
        ResourceDescription description = this.configurator.getResourceDescription();
        ...
        ResourceDescriptionResolver resolver = JGroupsSubsystemResourceDefinitionRegistrar.RESOLVER.createChildResolver(description.getPathElement());
        ...
        ManagementResourceRegistration registration = parent.registerSubModel(ResourceDefinition.builder(description, resolver).build());
    ...
    public void addRuntime(OperationContext context, ModelNode model) throws OperationFailedException {
        ...
        List<PathElement> protocolTypes = List.of(TransportResourceDescription.INSTANCE.getPathElement(), ProtocolResourceDescription.INSTANCE.getPathElement(), RelayResourceDescription.INSTANCE.getPathKey());
        if (protocolTypes.stream().anyMatch(protocolType -> stackResource.hasChildren(protocolType.getKey()))) {
            ...
            if (resource != stackResource) {
                ...
                OverrideDescriptionProvider provider = new OverrideDescriptionProvider() {
                    ...
                    public Map<String, ModelNode> getChildTypeOverrideDescriptions(Locale locale) {
                        String description = JGroupsSubsystemResourceDefinitionRegistrar.RESOLVER.getChildTypeDescription(ProtocolResourceDescription.INSTANCE.getPathElement().getKey(), locale, JGroupsSubsystemResourceDefinitionRegistrar.RESOLVER.getResourceBundle(locale));
                        ModelNode result = new ModelNode();
                        result.get(ModelDescriptionConstants.DESCRIPTION).set(description);
                        return Collections.singletonMap(ProtocolResourceDescription.INSTANCE.getPathElement().getKey(), result);
                    }
                };
            ...
            for (PathElement path : protocolTypes) {
                for (Resource.ResourceEntry protocolResource : stackResource.getChildren(path.getKey())) {
                    ...
                    resource.registerChild(ProtocolResourceDescription.pathElement(protocolName), PlaceholderResource.INSTANCE);
                }
            }
        }
    }
    ...
    private ManagementResourceRegistration register(ManagementResourceRegistration parent, String protocolName, Class<? extends Protocol> protocolClass) {
        ...
        ProtocolResourceDescription description = ProtocolResourceDescription.of(protocolName);
        ManagementResourceRegistration registration = parent.getSubModel(PathAddress.pathAddress(description.getPathElement()));
        if (registration == null) {
            ...
            ResourceDefinition definition = ResourceDefinition.builder(description, resolver).asRuntime().build();
    ...
    static Class<? extends Protocol> findProtocolClass(OperationContext context, String protocolName, ModelNode protocolModel) throws OperationFailedException {
        String moduleName = ProtocolChildResourceDescription.MODULE.resolveModelAttribute(context, protocolModel).asString();
        String className = protocolName;
        if (moduleName.equals(ProtocolChildResourceDescription.MODULE.getDefaultValue().asString()) && !protocolName.startsWith(Global.PREFIX)) {

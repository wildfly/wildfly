...
public class JGroupsSubsystemResourceDefinitionRegistrar implements SubsystemResourceDefinitionRegistrar, BiPredicate<OperationContext, Resource>, ResourceServiceConfigurator {
    ...
    static final ParentResourceDescriptionResolver RESOLVER = new SubsystemResourceDescriptionResolver(JGroupsSubsystemResourceDescription.INSTANCE.getName(), JGroupsSubsystemResourceDefinitionRegistrar.class);
    ...
    public boolean test(OperationContext context, Resource resource) {
        return resource.getModel().hasDefined(JGroupsSubsystemResourceDescription.DEFAULT_CHANNEL.getName());
    }
    ...
    public ManagementResourceRegistration register(SubsystemRegistration parent, ManagementResourceRegistrationContext context) {
        ...
        ResourceDescriptor descriptor = ResourceDescriptor.builder(RESOLVER)
                .addAttributes(JGroupsSubsystemResourceDescription.INSTANCE.getAttributes().collect(Collectors.toList()))
                .addCapabilities(List.of(JGroupsSubsystemResourceDescription.DEFAULT_CHANNEL_CAPABILITY, DEFAULT_CHANNEL_CONFIGURATION_CAPABILITY, DEFAULT_CHANNEL_FACTORY_CAPABILITY, defaultCommandDispatcherFactory, defaultLegacyCommandDispatcherFactory, defaultGroup, defaultLegacyGroup), this)
                .withRuntimeHandler(ResourceOperationRuntimeHandler.configureService(this))
                .build();

        ManagementResourceRegistration registration = parent.registerSubsystemModel(ResourceDefinition.builder(JGroupsSubsystemResourceDescription.INSTANCE, RESOLVER).build());
    ...
    public ResourceServiceInstaller configure(OperationContext context, ModelNode model) throws OperationFailedException {
        ...
        String defaultChannel = JGroupsSubsystemResourceDescription.DEFAULT_CHANNEL.resolveModelAttribute(context, model).asStringOrNull();
        if (defaultChannel != null) {
            installers.add(CapabilityServiceInstaller.builder(JGroupsSubsystemResourceDescription.DEFAULT_CHANNEL_CAPABILITY, ServiceDependency.on(JGroupsServiceDescriptor.CHANNEL, defaultChannel)).build());

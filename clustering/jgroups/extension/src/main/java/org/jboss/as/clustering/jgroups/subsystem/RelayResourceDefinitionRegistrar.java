...
public class RelayResourceDefinitionRegistrar extends ProtocolConfigurationResourceDefinitionRegistrar<RELAY2, RelayConfiguration> {
    ...
    private final ProtocolConfigurationResourceDescriptorConfigurator<RELAY2, RelayConfiguration> configurator;

    RelayResourceDefinitionRegistrar(ResourceOperationRuntimeHandler parentRuntimeHandler) {
        this(new ProtocolConfigurationResourceDescriptorConfigurator<>() {
            @Override
            public ProtocolChildResourceDescription getResourceDescription() {
                return RelayResourceDescription.INSTANCE;
            }

            @Override
            public ResourceDescriptionResolver getResourceDescriptionResolver() {
                return JGroupsSubsystemResourceDefinitionRegistrar.RESOLVER.createChildResolver(RelayResourceDescription.INSTANCE.getPathKey(), ProtocolResourceDescription.INSTANCE.getPathElement());
            }
            ...
            public ServiceDependency<RelayConfiguration> resolve(OperationContext context, ModelNode model) throws OperationFailedException {
                ...
                String siteName = RelayResourceDescription.Attribute.SITE.resolveModelAttribute(context, model).asString();

                Resource resource = context.readResource(PathAddress.EMPTY_ADDRESS);
                Set<String> remoteSiteNames = resource.getChildrenNames(RemoteSiteResourceDescription.INSTANCE.getPathElement().getKey());
    ...
    private RelayResourceDefinitionRegistrar(ProtocolConfigurationResourceDescriptorConfigurator<RELAY2, RelayConfiguration> configurator) {
        super(configurator);
    ...
    public ManagementResourceRegistration register(ManagementResourceRegistration parent, ManagementResourceRegistrationContext context) {
        ...
        new RemoteSiteResourceDefinitionRegistrar(ResourceOperationRuntimeHandler.configureService(this.configurator)).register(registration, context);

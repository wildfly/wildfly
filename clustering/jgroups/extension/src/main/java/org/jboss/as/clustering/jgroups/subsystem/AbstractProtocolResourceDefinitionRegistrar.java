...
public class AbstractProtocolResourceDefinitionRegistrar<P extends Protocol> extends ProtocolConfigurationResourceDefinitionRegistrar<P, ProtocolConfiguration<P>> {
        ...
        public OperationStepHandler apply(OperationStepHandler handler) {
            return new OperationStepHandler() {
                ...
                public void execute(OperationContext context, ModelNode operation) throws OperationFailedException {
                    ...
                    PathElement nativeProtocolPath = ProtocolResourceDescription.legacy(protocolPath).getPathElement();
    ...
    static class LegacyAddOperationTransformation implements UnaryOperator<OperationStepHandler> {
        ...
        LegacyAddOperationTransformation(String... legacyProperties) {
            ...
            this.legacy = operation -> {
                if (!operation.hasDefined(ProtocolChildResourceDescription.PROPERTIES.getName())) return false;
                for (String legacyProperty : legacyProperties) {
                    if (operation.get(ProtocolChildResourceDescription.PROPERTIES.getName()).hasDefined(legacyProperty)) return true;
                }
                return false;
            };
        }
        ...
        public OperationStepHandler apply(OperationStepHandler handler) {
            return (context, operation) -> {
                if (this.legacy.test(operation)) {
                    ...
                    OperationStepHandler genericHandler = context.getResourceRegistration().getParent().getOperationHandler(PathAddress.pathAddress(ProtocolResourceDescription.INSTANCE.getPathElement()), ModelDescriptionConstants.ADD);
                    operation.get(ModelDescriptionConstants.OP_ADDR).set(context.getCurrentAddress().getParent().append(ProtocolResourceDescription.legacyPathElement(path.getValue())).toModelNode());
    ...
    interface ProtocolResourceDescriptorConfigurator<P extends Protocol> extends ProtocolConfigurationResourceDescriptorConfigurator<P, ProtocolConfiguration<P>> {
        @Override
        ProtocolResourceDescription getResourceDescription();
        ...
        default ResourceDescriptionResolver getResourceDescriptionResolver() {
            ...
            return path.isWildcard() ? JGroupsSubsystemResourceDefinitionRegistrar.RESOLVER.createChildResolver(path) : JGroupsSubsystemResourceDefinitionRegistrar.RESOLVER.createChildResolver(path, ProtocolResourceDescription.INSTANCE.getPathElement());
        }
        ...
        @Override
        default ServiceDependency<ProtocolConfiguration<P>> resolve(OperationContext context, ModelNode model) throws OperationFailedException {
            return this.getProtocolConfigurationResolver().resolve(context, model);
        }
    }

    AbstractProtocolResourceDefinitionRegistrar(ProtocolResourceDescriptorConfigurator<P> registration) {
        super(registration);
    }
}

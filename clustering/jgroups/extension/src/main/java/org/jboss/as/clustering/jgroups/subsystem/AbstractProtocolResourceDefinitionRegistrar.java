...
public class AbstractProtocolResourceDefinitionRegistrar<P extends Protocol> extends ProtocolConfigurationResourceDefinitionRegistrar<P, ProtocolConfiguration<P>> {
    ...
    static Stream<AttributeDefinition> attributes() {
        return ProtocolChildResourceDefinitionRegistrar.attributes();
    }
        ...
        public OperationStepHandler apply(OperationStepHandler handler) {
            return new OperationStepHandler() {
                ...
                public void execute(OperationContext context, ModelNode operation) throws OperationFailedException {
                    ...
                    PathElement nativeProtocolPath = NativeProtocolResourceDefinitionRegistrar.pathElement(protocolPath.getValue());
    ...
    static class LegacyAddOperationTransformation implements UnaryOperator<OperationStepHandler> {
        ...
        LegacyAddOperationTransformation(String... legacyProperties) {
            ...
            this.legacy = operation -> {
                if (!operation.hasDefined(PROPERTIES.getName())) return false;
                for (String legacyProperty : legacyProperties) {
                    if (operation.get(PROPERTIES.getName()).hasDefined(legacyProperty)) return true;
                }
                return false;
            };
        }
        ...
        public OperationStepHandler apply(OperationStepHandler handler) {
            return (context, operation) -> {
                if (this.legacy.test(operation)) {
                    ...
                    OperationStepHandler genericHandler = context.getResourceRegistration().getParent().getOperationHandler(PathAddress.pathAddress(ProtocolResourceDefinitionRegistrar.WILDCARD_PATH), ModelDescriptionConstants.ADD);
                    operation.get(ModelDescriptionConstants.OP_ADDR).set(context.getCurrentAddress().getParent().append(NativeProtocolResourceDefinitionRegistrar.pathElement(path.getValue())).toModelNode());
    ...
    interface ProtocolResourceRegistration<P extends Protocol> extends ProtocolConfigurationResourceRegistration<P, ProtocolConfiguration<P>> {
        @Override
        ...
        default ResourceDescriptionResolver getResourceDescriptionResolver() {
            ...
            return path.isWildcard() ? JGroupsSubsystemResourceDefinitionRegistrar.RESOLVER.createChildResolver(path) : JGroupsSubsystemResourceDefinitionRegistrar.RESOLVER.createChildResolver(path, pathElement(PathElement.WILDCARD_VALUE));
        }
    ...
    AbstractProtocolResourceDefinitionRegistrar(ProtocolResourceRegistration<P> registration) {
        super(registration);
    }
}

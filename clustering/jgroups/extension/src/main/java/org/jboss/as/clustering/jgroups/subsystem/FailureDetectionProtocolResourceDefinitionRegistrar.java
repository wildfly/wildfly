 ...
 * @author Paul Ferraro
 */
public class FailureDetectionProtocolResourceDefinitionRegistrar extends AbstractProtocolResourceDefinitionRegistrar<FD_SOCK2> {

    static final ResourceCapabilityReference<TransportConfiguration<TP>> TRANSPORT = ResourceCapabilityReference.builder(ProtocolResourceDescription.CAPABILITY, TransportConfiguration.SERVICE_DESCRIPTOR).withRequirementNameResolver(UnaryCapabilityNameResolver.PARENT).build();

    FailureDetectionProtocolResourceDefinitionRegistrar(SocketProtocolResourceDescription description, ResourceOperationRuntimeHandler parentRuntimeHandler) {
        super(new ProtocolResourceDescriptorConfigurator<>() {
            ...
            public ResourceDescriptor.Builder apply(ResourceDescriptor.Builder builder) {
                return ProtocolResourceDescriptorConfigurator.super.apply(builder).addResourceCapabilityReference(TRANSPORT);
            }
            ...
            public ServiceDependency<ProtocolConfiguration<FD_SOCK2>> resolve(OperationContext context, ModelNode model) throws OperationFailedException {
                ServiceDependency<ProtocolConfiguration<FD_SOCK2>> protocol = ProtocolResourceDescriptorConfigurator.super.resolve(context, model);
                ServiceDependency<SocketBinding> serverBinding = SocketProtocolResourceDescription.SocketBindingAttribute.SERVER.resolve(context, model);
                ServiceDependency<SocketBinding> clientBinding = SocketProtocolResourceDescription.SocketBindingAttribute.CLIENT.resolve(context, model);

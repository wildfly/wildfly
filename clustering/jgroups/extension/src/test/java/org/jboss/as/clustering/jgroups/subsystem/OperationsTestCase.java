...
public class OperationsTestCase extends OperationTestCaseBase {
    ...
    public void testTransportReadWriteOperation() throws Exception {
        ...
        ModelNode result = services.executeOperation(getTransportReadOperation("maximal", "TCP", AbstractTransportResourceDefinitionRegistrar.Attribute.RACK.get()));
        ...
        result = services.executeOperation(getTransportWriteOperation("maximal", "TCP", AbstractTransportResourceDefinitionRegistrar.Attribute.RACK.get(), "new-rack"));
        Assert.assertEquals(result.toString(), SUCCESS, result.get(OUTCOME).asString());

        // re-read the rack attribute
        result = services.executeOperation(getTransportReadOperation("maximal", "TCP", AbstractTransportResourceDefinitionRegistrar.Attribute.RACK.get()));
        Assert.assertEquals(result.toString(), SUCCESS, result.get(OUTCOME).asString());
    ...
    public void testProtocolReadWriteOperation() throws Exception {
        ...
        result = services.executeOperation(getProtocolReadOperation("maximal", "MPING", MulticastProtocolResourceDefinitionRegistrar.SOCKET_BINDING));
        Assert.assertEquals(result.toString(), SUCCESS, result.get(OUTCOME).asString());
        Assert.assertEquals("jgroups-mping", result.get(RESULT).asString());

        // write the attribute
        result = services.executeOperation(getProtocolWriteOperation("maximal", "MPING", MulticastProtocolResourceDefinitionRegistrar.SOCKET_BINDING, "new-socket-binding"));
        Assert.assertEquals(result.toString(), SUCCESS, result.get(OUTCOME).asString());

...
public class OperationsTestCase extends OperationTestCaseBase {
    ...
    public void testSubsystemReadWriteOperations() throws Exception {
        ...
        ModelNode result = services.executeOperation(getSubsystemReadOperation(JGroupsSubsystemResourceDescription.DEFAULT_CHANNEL));
        Assert.assertEquals(result.get(FAILURE_DESCRIPTION).asString(), SUCCESS, result.get(OUTCOME).asString());
        Assert.assertEquals("ee", result.get(RESULT).asString());

        // write the default stack
        result = services.executeOperation(getSubsystemWriteOperation(JGroupsSubsystemResourceDescription.DEFAULT_CHANNEL, "bridge"));
        Assert.assertEquals(result.get(FAILURE_DESCRIPTION).asString(), SUCCESS, result.get(OUTCOME).asString());

        // re-read the default stack
        result = services.executeOperation(getSubsystemReadOperation(JGroupsSubsystemResourceDescription.DEFAULT_CHANNEL));
        Assert.assertEquals(result.get(FAILURE_DESCRIPTION).asString(), SUCCESS, result.get(OUTCOME).asString());
    ...
    public void testTransportReadWriteOperation() throws Exception {
        ...
        ModelNode result = services.executeOperation(getTransportReadOperation("maximal", "TCP", TransportResourceDescription.TopologyAttribute.RACK.get()));
        ...
        result = services.executeOperation(getTransportWriteOperation("maximal", "TCP", TransportResourceDescription.TopologyAttribute.RACK.get(), "new-rack"));
        Assert.assertEquals(result.toString(), SUCCESS, result.get(OUTCOME).asString());

        // re-read the rack attribute
        result = services.executeOperation(getTransportReadOperation("maximal", "TCP", TransportResourceDescription.TopologyAttribute.RACK.get()));
        Assert.assertEquals(result.toString(), SUCCESS, result.get(OUTCOME).asString());
    ...
    public void testProtocolReadWriteOperation() throws Exception {
        ...
        result = services.executeOperation(getProtocolReadOperation("maximal", "MPING", MulticastProtocolResourceDescription.SOCKET_BINDING));
        Assert.assertEquals(result.toString(), SUCCESS, result.get(OUTCOME).asString());
        Assert.assertEquals("jgroups-mping", result.get(RESULT).asString());

        // write the attribute
        result = services.executeOperation(getProtocolWriteOperation("maximal", "MPING", MulticastProtocolResourceDescription.SOCKET_BINDING, "new-socket-binding"));
        Assert.assertEquals(result.toString(), SUCCESS, result.get(OUTCOME).asString());

...
public class JGroupsSubsystemTestCase extends AbstractSubsystemSchemaTest<JGroupsSubsystemSchema> {
    ...
    public JGroupsSubsystemTestCase(JGroupsSubsystemSchema schema) {
        super(JGroupsSubsystemResourceDescription.INSTANCE.getName(), new JGroupsExtension(), schema, JGroupsSubsystemSchema.CURRENT);
        this.schema = schema;
    }
    ...
    public void testIndexedAdds() throws Exception {
        ...
        ModelNode originalSubsystemModel = services.readWholeModel().get(JGroupsSubsystemResourceDescription.INSTANCE.getPathElement().getKeyValuePair());
        ModelNode originalChannelModel = originalSubsystemModel.get(ChannelResourceDescription.pathElement("ee").getKeyValuePair());
        ModelNode originalForkModel = originalChannelModel.get(ForkResourceDescription.pathElement("web").getKeyValuePair());

        Assert.assertTrue(originalForkModel.isDefined());
        originalForkModel.protect();
        Assert.assertTrue(0 < originalForkModel.get(ProtocolResourceDescription.INSTANCE.getPathElement().getKey()).keys().size());

        ModelNode originalStackModel = originalSubsystemModel.get(StackResourceDescription.pathElement("maximal").getKeyValuePair());
        Assert.assertTrue(originalStackModel.isDefined());
        originalStackModel.protect();


        final PathAddress subsystemAddress = PathAddress.pathAddress(JGroupsSubsystemResourceDescription.INSTANCE.getPathElement());
        final PathAddress forkAddress = subsystemAddress.append(ChannelResourceDescription.pathElement("ee")).append(ForkResourceDescription.pathElement("web"));
        final PathAddress stackAddress = subsystemAddress.append(StackResourceDescription.pathElement("maximal"));

        //Check the fork protocols honour indexed adds by inserting a protocol at the start
        ModelNode add = Util.createAddOperation(forkAddress.append(ProtocolResourceDescription.pathElement("MERGE3")), 0);
        ModelTestUtils.checkOutcome(services.executeOperation(add));

        ModelNode subsystemModel = services.readWholeModel().get(JGroupsSubsystemResourceDescription.INSTANCE.getPathElement().getKeyValuePair());
        ModelNode channelModel = subsystemModel.get(ChannelResourceDescription.pathElement("ee").getKeyValuePair());
        ModelNode forkModel = channelModel.get(ForkResourceDescription.pathElement("web").getKeyValuePair());

        Assert.assertEquals(originalForkModel.keys().size() + 1, forkModel.get(ProtocolResourceDescription.INSTANCE.getPathElement().getKey()).keys().size());
        Assert.assertEquals("MERGE3", forkModel.get(ProtocolResourceDescription.INSTANCE.getPathElement().getKey()).keys().iterator().next());

        //Check the stack protocols honour indexed adds by removing a protocol in the middle and re-adding it
        ModelNode remove = Util.createRemoveOperation(stackAddress.append(ProtocolResourceDescription.pathElement("FD")));
        ModelTestUtils.checkOutcome(services.executeOperation(remove));
        add = Util.createAddOperation(stackAddress.append(ProtocolResourceDescription.pathElement("FD")), 3); //The original index of the FD protocol
        ModelTestUtils.checkOutcome(services.executeOperation(add));

        subsystemModel = services.readWholeModel().get(JGroupsSubsystemResourceDescription.INSTANCE.getPathElement().getKeyValuePair());
        ModelNode stackModel = subsystemModel.get(StackResourceDescription.pathElement("maximal").getKeyValuePair());

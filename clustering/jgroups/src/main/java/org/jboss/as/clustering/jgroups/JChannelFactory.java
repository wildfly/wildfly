...
public class JChannelFactory implements ChannelFactory, ProtocolStackConfigurator {
    ...
    public List<org.jgroups.conf.ProtocolConfiguration> getProtocolStack() {
        ...
        boolean multicastCapable = isMulticastCapable(transportClass);
        ...
        for (ProtocolConfiguration protocol: this.configuration.getProtocols()) {
            ...
            if (!multicastCapable) {
                setPropertyNoOverride(config, protocolClass, "use_mcast_xmit", String.valueOf(false));
                setPropertyNoOverride(config, protocolClass, "use_mcast_xmit_req", String.valueOf(false));
            }
    ...
    private static boolean isMulticastCapable(Class<? extends TP> transportClass) {
        try {
            return transportClass.newInstance().isMulticastCapable();

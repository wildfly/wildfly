...
public class JGroupsSubsystemXMLReader_2_0 implements XMLElementReader<List<ModelNode>> {
    ...
    private static void parseStack(XMLExtendedStreamReader reader, PathAddress subsystemAddress, List<ModelNode> operations) throws XMLStreamException {
        ...
        while (reader.hasNext() && (reader.nextTag() != XMLStreamConstants.END_ELEMENT)) {
            ...
            switch (element) {
                ...
                case RELAY: {
                    parseRelay(reader, stackAddress, operations);
                    break;
                }
                default: {
                    throw ParseUtils.unexpectedElement(reader);
    ...
    private static void parseRelay(XMLExtendedStreamReader reader, final PathAddress stackAddress, List<ModelNode> operations) throws XMLStreamException {
        ...
        while (reader.hasNext() && (reader.nextTag() != XMLStreamConstants.END_ELEMENT)) {
            ...
            switch (element) {
                case REMOTE_SITE: {
                    parseRemoteSite(reader, relayAddress, operations);
                    break;
                }
                case PROPERTY: {
                    parseProperty(reader, relayAddress, operations);
                    break;
                }
                default: {
                    throw ParseUtils.unexpectedElement(reader);
    ...
    private static void parseRemoteSite(XMLExtendedStreamReader reader, final PathAddress relayAddress, List<ModelNode> operations) throws XMLStreamException {
        String site = null;
        String stack = null;
        String cluster = null;
        ...
        PathAddress siteAddress = relayAddress.append(RemoteSiteResourceDefinition.pathElement(site));
        ModelNode operation = Util.createAddOperation(siteAddress);
        operations.add(operation);

        RemoteSiteResourceDefinition.STACK.parseAndSetParameter(stack, operation, reader);
        RemoteSiteResourceDefinition.CLUSTER.parseAndSetParameter(cluster, operation, reader);

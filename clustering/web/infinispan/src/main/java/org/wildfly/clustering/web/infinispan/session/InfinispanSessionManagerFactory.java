    ...
    private <L> SessionFactory<?, L> getSessionFactory(LocalContextFactory<L> localContextFactory) {
        ...
        switch (config.getAttributePersistenceStrategy()) {
            case FINE: {
                ...
                return new FineSessionFactory<>(cache, marshaller, localContextFactory, lockOnRead);
            }
            case COARSE: {
                ...
                return new CoarseSessionFactory<>(cache, marshaller, localContextFactory, lockOnRead);
            }
            default: {
                // Impossible

...
public class DistributableSession extends AbstractDistributableSession {
    ...
    public DistributableSession(UndertowSessionManager manager, Session<Map<String, Object>> session, Consumer<HttpServerExchange> closeTask) {
        this(manager, BlockingReference.of(session), closeTask, session.getMetaData());
    }

    private DistributableSession(UndertowSessionManager manager, BlockingReference<Session<Map<String, Object>>> reference, Consumer<HttpServerExchange> closeTask, ImmutableSessionMetaData metaData) {
        super(manager, reference);
        this.manager = manager;
        this.reference = reference;
        this.closeTask = new AtomicReference<>(closeTask);
        this.startTime = metaData.getLastAccessStartTime().isEmpty() ? metaData.getCreationTime() : Instant.now();
    }
    ...
    public void requestDone(HttpServerExchange exchange) {
        ...
        if (closeTask != null) {
            this.reference.getReader().read(completeSession -> {
                ...
                try (Session<Map<String, Object>> session = completeSession) {
                    if (session.isValid()) {
    ...
    public String changeSessionId(HttpServerExchange exchange, SessionConfig config) {
        ...
        String currentId = this.reference.getWriter(Session::isValid).updateAndGet(new UnaryOperator<>() {
            @Override

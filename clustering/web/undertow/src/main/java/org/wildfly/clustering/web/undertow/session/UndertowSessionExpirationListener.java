...
public class UndertowSessionExpirationListener implements SessionExpirationListener {
    ...
    public void sessionExpired(ImmutableSession session) {
        ...
        // Perform session listener callback within a separate batch context
        Batch batch = batcher.suspendBatch();
        try {
            this.listeners.sessionDestroyed(new DistributableImmutableSession(manager, session), null, SessionListener.SessionDestroyedReason.TIMEOUT);
        } finally {

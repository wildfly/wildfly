...
public class DistributableSessionManagerFactory implements io.undertow.servlet.api.SessionManagerFactory {
    ...
    public io.undertow.server.session.SessionManager createSessionManager(final Deployment deployment) {
        ...
        final SessionManager<LocalSessionContext, Batch> manager = this.factory.createSessionManager(configuration);
        ThreadSetupAction action = new ThreadSetupAction() {
            @Override
            public Handle setup(HttpServerExchange exchange) {
                final Batch existingBatch = manager.getBatcher().suspendBatch();
                return new Handle() {
                    @Override
                    public void tearDown() {
                        ...
                        Batch residualBatch = manager.getBatcher().suspendBatch();
                        if (residualBatch != null) {
                            UndertowLogger.REQUEST_LOGGER.tracef("Suspending residual active batch: %s", residualBatch);
                        }
                        if (existingBatch != null) {
                            manager.getBatcher().resumeBatch(existingBatch);
                        }
                    }
                };
            }

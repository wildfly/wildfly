...
public class DistributableWebDeploymentServiceInstallerProvider implements WebDeploymentServiceInstallerProvider {
    ...
    public DeploymentServiceInstaller getSessionManagerFactoryServiceInstaller(org.wildfly.clustering.web.container.SessionManagerFactoryConfiguration configuration) {
        ...
        ExceptionFunction<String, Class<?>, ClassNotFoundException> loader = module.getClassLoader()::loadClass;
        List<String> immutableClasses = unit.getAttachmentList(DistributableSessionManagementProvider.IMMUTABILITY_ATTACHMENT_KEY);
        Immutability immutability = Immutability.classes(immutableClasses.stream().map(Functions.quiet(loader, IllegalArgumentException::new)).toList());
    ...
    private DistributableSessionManagementProvider findDistributableSessionManagementProvider(DeploymentUnit unit) {
        ...
        if ((metaData.getReplicationConfig() != null) || (provider == null)) {
            if (provider != null) {
            ...
            return this.legacyFactory.createSessionManagerProvider(unit, metaData.getReplicationConfig());
        }
        return provider;
    }

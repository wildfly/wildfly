...
public class InfinispanBeanGroupFactory<G, I, T> implements BeanGroupFactory<G, I, T> {
    ...
    public BeanGroup<G, I, T> createGroup(final G id, final BeanGroupEntry<I, T> entry) {
        final Cache<G, BeanGroupEntry<I, T>> cache = this.cache;
        final CacheInvoker invoker = this.invoker;
        Mutator mutator = new Mutator() {
            @Override
            public void mutate() {
                invoker.invoke(cache, new MutateOperation<>(id, entry), Flag.IGNORE_RETURN_VALUES);
            }
        };

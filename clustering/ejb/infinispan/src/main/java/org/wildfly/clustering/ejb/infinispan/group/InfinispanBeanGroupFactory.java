...
public class InfinispanBeanGroupFactory<I, T> implements BeanGroupFactory<I, T> {
    ...
    public void evict(I id) {
        ...
        BeanGroupEntry<I, T> entry = this.findCache.getAdvancedCache().withFlags(Flag.CACHE_MODE_LOCAL, Flag.SKIP_CACHE_LOAD, Flag.ZERO_LOCK_ACQUISITION_TIMEOUT, Flag.FAIL_SILENTLY).get(key);
        if (entry != null) {
            try {

...
public class InfinispanBeanFactory<G, I, T> implements BeanFactory<G, I, T> {
    ...
    public Bean<G, I, T> createBean(I id, BeanEntry<G> entry) {
        ...
        return new InfinispanBean<>(id, entry, group, this, this.timeout, this.listener);
    }

    @Override
    public BeanEntry<G> findValue(I id) {
        return this.invoker.invoke(this.beanCache, new FindOperation<BeanKey<I>, BeanEntry<G>>(this.createKey(id)));
    }

    @Override
    public BeanEntry<G> createValue(I id, G groupId) {
        BeanEntry<G> entry = new InfinispanBeanEntry<>(groupId);
        BeanEntry<G> existing = this.invoker.invoke(this.beanCache, new Creator.CreateOperation<>(this.createKey(id), entry));
        return (existing != null) ? existing : entry;
    }

    @Override
    public void remove(I id, RemoveListener<T> listener) {
        BeanEntry<G> entry = this.invoker.invoke(this.beanCache, new RemoveOperation<BeanKey<I>, BeanEntry<G>>(this.createKey(id)));
        if (entry != null) {
            G groupId = entry.getGroupId();
    ...
    public void evict(I id) {
        ...
        BeanEntry<G> entry = this.invoker.invoke(this.beanCache, new Locator.FindOperation<BeanKey<I>, BeanEntry<G>>(key));
        if (entry != null) {

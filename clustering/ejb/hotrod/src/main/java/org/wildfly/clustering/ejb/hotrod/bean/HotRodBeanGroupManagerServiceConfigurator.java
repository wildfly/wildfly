 ...
 * @author Paul Ferraro
 */
public class HotRodBeanGroupManagerServiceConfigurator<K, V extends BeanInstance<K>> extends BeanGroupManagerServiceNameProvider implements CapabilityServiceConfigurator, Supplier<BeanGroupManager<K, V>>, HotRodConfiguration {

    private final SupplierDependency<RemoteCache<?, ?>> cache;
    private final SupplierDependency<ByteBufferMarshaller> marshaller;

    public HotRodBeanGroupManagerServiceConfigurator(DeploymentConfiguration config, SupplierDependency<RemoteCache<?, ?>> cache, SupplierDependency<ByteBufferMarshaller> marshaller) {
        super(config);
        this.cache = cache;
    ...
    public BeanGroupManager<K, V> get() {
        ...
        HotRodBeanGroupManager<K, V, ByteBufferMarshaller> factory = new HotRodBeanGroupManager<>(this);
    ...
    public <CK, CV> RemoteCache<CK, CV> getCache() {
        return (RemoteCache<CK, CV>) this.cache.get();
    }
}

...
interface ClusteringImplLogger extends ClusteringLogger {
    ...
    @Message(id = 10230, value = "Error destroying service")
    ...
    @Message(id = 10231, value = "Exception in stop")
     ...
     */
    @LogMessage(level = WARN)
    @Message(value = "Partition %s message wrapper does not contain Object[] object!")
     ...
     */
    @LogMessage(level = WARN)
    @Message(value = "Partition %s message does not contain a MethodCall object!")
     ...
     */
    @LogMessage(level = WARN)
    @Message(id = 10232, value = "Membership listener callback failure: %s")
     ...
     */
    @LogMessage(level = ERROR)
    @Message(id = 10233, value = "%s failed")
     ...
     */
    @LogMessage(level = ERROR)
    @Message(id = 10234, value = "%s failed for service %s")
    ...
    @Message(id = 10235, value = "New cluster view for partition %s (id: %d, delta: %d, merge: %b) : %s")
     ...
     */
    @LogMessage(level = INFO)
    @Message(id = 10236, value = "New cluster view for partition %s: %d (%s delta: %d, merge: %b)")
     ...
     */
    @LogMessage(level = WARN)
    @Message(id = 10237, value = "No %s registered to receive state for service %s")
     ...
     */
    @LogMessage(level = WARN)
    @Message(value = "Partition %s message or message buffer is null!")
     ...
     */
    @LogMessage(level = WARN)
    @Message(value = "Partition %s failed extracting message body from request bytes")
     ...
     */
    @LogMessage(level = WARN)
    @Message(value = "Partition %s failed unserializing message buffer (msg=%s)")
     ...
     */
    @LogMessage(level = WARN)
    @Message(id = 10238, value = "Received concurrent requests to get service state for %s")
     ...
     */
    @LogMessage(level = INFO)
    @Message(id = 10239, value = "Suspected member: %s")

...
import static org.wildfly.extension.cluster.ModelKeys.NAME;
...
public class ClusterSubsystemRootResource implements Resource {
    ...
    public boolean hasChildren(String childType) {
        if (NAME.equals(childType)) {
            return getChildrenNames(NAME).size() > 0;
        } else {
    ...
    public Resource navigate(PathAddress address) {
        if (address.size() > 0 && ModelKeys.NAME.equals(address.getElement(0).getKey())) {
            // resource too deep
            if (address.size() > 1) {
    ...
    public Set<ResourceEntry> getChildren(String childType) {
        if (NAME.equals(childType)) {
            ...
            for (ServiceName serviceName : ClusterSubsystemHelper.getChannelServiceNames(this.registry)) {
                ...
                String name = ClusterSubsystemHelper.getChannelNameFromChannelServiceName(serviceName);

                ClusterInstanceResource.ClusterInstanceResourceEntry entry =
                        new ClusterInstanceResource.ClusterInstanceResourceEntry(childType, name);
    ...
    private boolean hasChannel(PathElement element) {
        ...
        assert element.getKey().equals(ModelKeys.NAME);

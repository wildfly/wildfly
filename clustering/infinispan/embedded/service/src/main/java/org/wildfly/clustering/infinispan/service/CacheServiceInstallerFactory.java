    ...
    public ServiceInstaller apply(BinaryServiceConfiguration configuration) {
        ...
        Supplier<Cache<?, ?>> cache = new Supplier<>() {
            ...
            public Cache<?, ?> get() {
                ...
                // For replicated/invalidation caches, use zero capacity
                // For distributed caches, use smallest positive non-zero capacity.
                Configuration suspendedConfiguration = originalConfiguration.clustering().cacheMode().isClustered() ? new ConfigurationBuilder().read(originalConfiguration).clustering().hash().capacityFactor(originalConfiguration.clustering().cacheMode().isDistributed() ? Float.MIN_VALUE : 0f).build() : originalConfiguration;
    ...
    private static class DefaultCache<K, V> extends AbstractDelegatingAdvancedCache<K, V> {
        ...
        DefaultCache(Cache<K, V> cache, Configuration resumedConfiguration, Configuration suspendedConfiguration, SuspendableActivityRegistry activityRegistry) {
            this(cache.getAdvancedCache(), activityRegistry, new SuspendableActivity() {
                private final BlockingManager blocking = cache.getCacheManager().getGlobalComponentRegistry().getComponent(BlockingManager.class);
                @Override
                public CompletionStage<Void> suspend(ServerSuspendContext context) {
                    if (context.isStarting() || context.isStopping() || (resumedConfiguration == suspendedConfiguration) || (cache.getCacheConfiguration().clustering().hash().capacityFactor() == suspendedConfiguration.clustering().hash().capacityFactor())) {
                    ...
                    return this.blocking.runBlocking(new Runnable() {
                        ...
                        public void run() {
                            ...
                            cache.getAdvancedCache().getComponentRegistry().getComponent(BasicComponentRegistry.class).replaceComponent(Configuration.class.getName(), suspendedConfiguration, false);
                            cache.stop();
                            cache.start();
                        }
                    }, "suspend");
                ...
                public CompletionStage<Void> resume(ServerResumeContext context) {
                    ...
                    return this.blocking.runBlocking(new Runnable() {
                        ...
                        public void run() {
                            ...
                            cache.getAdvancedCache().getComponentRegistry().getComponent(BasicComponentRegistry.class).replaceComponent(Configuration.class.getName(), resumedConfiguration, false);
                            cache.stop();
                            cache.start();
                        }
                    }, "resume");

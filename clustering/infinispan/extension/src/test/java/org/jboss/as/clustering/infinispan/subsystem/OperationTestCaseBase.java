...
public class OperationTestCaseBase extends AbstractSubsystemTest {
    ...
    protected static ModelNode getCacheContainerAddOperation(String containerName) {
        ...
        return Util.createAddOperation(address, Map.of(CacheContainerResourceDescription.DEFAULT_CACHE.getName(), new ModelNode("default")));
    }
    ...
    protected static ModelNode getCacheStoreGetPropertyOperation(PathAddress cacheStoreAddress, String propertyName) {
        return Util.createMapGetOperation(cacheStoreAddress, StoreResourceDescription.PROPERTIES.getName(), propertyName);
    }
    ...
    protected static ModelNode getCacheStorePutPropertyOperation(PathAddress cacheStoreAddress, String propertyName, String propertyValue) {
        return Util.createMapPutOperation(cacheStoreAddress, StoreResourceDescription.PROPERTIES.getName(), propertyName, propertyValue);
    }

    protected static ModelNode getCacheStoreRemovePropertyOperation(PathAddress cacheStoreAddress, String propertyName) {
        return Util.createMapRemoveOperation(cacheStoreAddress, StoreResourceDescription.PROPERTIES.getName(), propertyName);
    }

    protected static ModelNode getCacheStoreClearPropertiesOperation(PathAddress cacheStoreAddress) {
        return Util.createMapClearOperation(cacheStoreAddress, StoreResourceDescription.PROPERTIES.getName());
    }

    protected static ModelNode getCacheStoreUndefinePropertiesOperation(PathAddress cacheStoreAddress) {
        return Util.getUndefineAttributeOperation(cacheStoreAddress, StoreResourceDescription.PROPERTIES.getName());
    }

    protected static PathAddress getJDBCCacheStoreAddress(String containerName, PathElement cachePath) {
        return getCacheAddress(containerName, cachePath).append(JDBCStoreResourceDescription.INSTANCE.getPathElement());
    }

    @SuppressWarnings("deprecation")
    protected static PathAddress getRemoteCacheStoreAddress(String containerName, PathElement cachePath) {
        return getCacheAddress(containerName, cachePath).append(RemoteStoreResourceDescription.INSTANCE.getPathElement());
    }

    protected static PathAddress getFileCacheStoreAddress(String containerName, PathElement cachePath) {
        return getCacheAddress(containerName, cachePath).append(FileStoreResourceDescription.INSTANCE.getPathElement());
    }

    protected static PathAddress getCustomCacheStoreAddress(String containerName, PathElement cachePath) {
        return getCacheAddress(containerName, cachePath).append(new CustomStoreResourceDescription<>().getPathElement());
    }

    protected static PathAddress getCacheContainerAddress(String containerName) {
        return PathAddress.pathAddress(InfinispanSubsystemResourceDescription.INSTANCE.getPathElement()).append(CacheContainerResourceDescription.pathElement(containerName));
    }

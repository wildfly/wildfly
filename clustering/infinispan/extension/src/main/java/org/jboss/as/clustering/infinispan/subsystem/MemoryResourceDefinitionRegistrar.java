...
public class MemoryResourceDefinitionRegistrar extends ConfigurationResourceDefinitionRegistrar<MemoryConfiguration, MemoryConfigurationBuilder> {
    ...
    interface Configurator extends ConfigurationResourceDefinitionRegistrar.Configurator {
        ...
        default ResourceDescriptor.Builder apply(ResourceDescriptor.Builder builder) {
            return ConfigurationResourceDefinitionRegistrar.Configurator.super.apply(builder)
                    .addAttributes(List.of(SIZE, this.getSizeUnitAttribute()));
        }
    }
    ...
    public ServiceDependency<MemoryConfigurationBuilder> resolve(OperationContext context, ModelNode model) throws OperationFailedException {
        StorageType storageType = this.configurator.getStorageType();
        long size = SIZE.resolveModelAttribute(context, model).asLong(-1L);
        MemorySizeUnit unit = this.configurator.getSizeUnitAttribute().resolve(context, model);

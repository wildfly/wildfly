...
public class InfinispanSubsystemResourceDefinitionRegistrar implements SubsystemResourceDefinitionRegistrar, Consumer<DeploymentProcessorTarget>, ResourceServiceConfigurator {
    public static final ParentResourceDescriptionResolver RESOLVER = new SubsystemResourceDescriptionResolver(InfinispanSubsystemResourceDescription.INSTANCE.getName(), InfinispanSubsystemResourceDefinitionRegistrar.class);
    ...
    static final RuntimeCapability<Void> DEFAULT_COMMAND_DISPATCHER_FACTORY = RuntimeCapability.Builder.of(ClusteringServiceDescriptor.DEFAULT_COMMAND_DISPATCHER_FACTORY).setAllowMultipleRegistrations(true).build();
    static final RuntimeCapability<Void> DEFAULT_GROUP = RuntimeCapability.Builder.of(ClusteringServiceDescriptor.DEFAULT_GROUP).setAllowMultipleRegistrations(true).build();
    ...
    public ManagementResourceRegistration register(SubsystemRegistration parent, ManagementResourceRegistrationContext context) {
        ...
        ResourceDescriptor descriptor = ResourceDescriptor.builder(RESOLVER)
                .addCapabilities(List.of(DEFAULT_COMMAND_DISPATCHER_FACTORY, DEFAULT_GROUP, localCommandDispatcherFactory, localGroup))
                .withDeploymentChainContributor(this)
                .withRuntimeHandler(ResourceOperationRuntimeHandler.configureService(this))
                .build();

        ManagementResourceRegistration registration = parent.registerSubsystemModel(ResourceDefinition.builder(InfinispanSubsystemResourceDescription.INSTANCE, RESOLVER).build());
    ...
    public void accept(DeploymentProcessorTarget target) {
        target.addDeploymentProcessor(InfinispanSubsystemResourceDescription.INSTANCE.getName(), Phase.DEPENDENCIES, Phase.DEPENDENCIES_CLUSTERING, new ClusteringDependencyProcessor());
    }
    ...
    public ResourceServiceInstaller configure(OperationContext context, ModelNode model) throws OperationFailedException {
        ...
        if (!context.getCapabilityServiceSupport().hasCapability(ChannelFactory.DEFAULT_SERVICE_DESCRIPTOR)) {
            installers.add(CapabilityServiceInstaller.builder(DEFAULT_COMMAND_DISPATCHER_FACTORY, ServiceDependency.on(ClusteringServiceDescriptor.COMMAND_DISPATCHER_FACTORY, ModelDescriptionConstants.LOCAL)).build());
            installers.add(CapabilityServiceInstaller.builder(DEFAULT_GROUP, ServiceDependency.on(ClusteringServiceDescriptor.GROUP, ModelDescriptionConstants.LOCAL)).build());
        }

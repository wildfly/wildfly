...
public enum InfinispanCacheContainerBindingFactory implements Function<String, ContextNames.BindInfo> {
    ...
    public ContextNames.BindInfo apply(String containerName) {
        return ContextNames.bindInfoFor(JndiNameFactory.createJndiName(JndiNameFactory.DEFAULT_JNDI_NAMESPACE, InfinispanSubsystemResourceDescription.INSTANCE.getName(), this.name, containerName).getAbsoluteName());
    }
}

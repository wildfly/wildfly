...
public class PartitionHandlingResourceDefinitionRegistrar extends ConfigurationResourceDefinitionRegistrar<PartitionHandlingConfiguration, PartitionHandlingConfigurationBuilder> {
    ...
    static final EnumAttributeDefinition<PartitionHandling> WHEN_SPLIT = EnumAttributeDefinition.nameBuilder("when-split", PartitionHandling.class)
            .setDefaultValue(PartitionHandling.ALLOW_READ_WRITES)
            .build();
    static final EnumAttributeDefinition<MergePolicy> MERGE_POLICY = EnumAttributeDefinition.nameBuilder("merge-policy", MergePolicy.class)
            .setDefaultValue(MergePolicy.NONE)
            .setAllowedValues(EnumSet.complementOf(EnumSet.of(MergePolicy.CUSTOM))).build();

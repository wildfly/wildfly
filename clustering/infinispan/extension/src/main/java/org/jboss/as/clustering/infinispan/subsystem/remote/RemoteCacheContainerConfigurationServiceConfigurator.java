...
public enum RemoteCacheContainerConfigurationServiceConfigurator implements ResourceServiceConfigurator {
    ...
    public ResourceServiceInstaller configure(OperationContext context, ModelNode model) throws OperationFailedException {
        ...
        Supplier<Configuration> configurationFactory = new Supplier<>() {
            ...
            private <C extends ConfigurationChildBuilder> void configureCluster(ConfigurationBuilder builder, ClusterConfigurator<C> configurator, Collection<ServiceDependency<OutboundSocketBinding>> bindings) {
                SslConfiguration ssl = security.get().ssl();
                C cluster = configurator.addCluster(builder);
                // Track unique source addresses.  Ideally, there are no more than one.
                Set<InetAddress> sourceAddresses = new HashSet<>(bindings.size());
                for (Supplier<OutboundSocketBinding> binding : bindings) {
                    OutboundSocketBinding socketBinding = binding.get();
                    sourceAddresses.add(socketBinding.getSourceAddress());
                    configurator.getBindingConsumer(cluster).accept(socketBinding.getUnresolvedDestinationAddress(), socketBinding.getDestinationPort());
                }
                if (ssl.enabled()) {

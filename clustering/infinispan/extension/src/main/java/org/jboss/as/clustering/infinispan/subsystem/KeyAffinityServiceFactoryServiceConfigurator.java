    ...
    public KeyAffinityServiceFactory apply(ExecutorService executor) {
        ...
        return new KeyAffinityServiceFactory() {
            ...
            public <K> KeyAffinityService<K> createService(Cache<K, ?> cache, KeyGenerator<K> generator) {
                ...
                return mode.isClustered() ? new KeyAffinityServiceImpl<>(executor, cache, generator, bufferSize, Collections.singleton(cache.getCacheManager().getAddress()), false) : new SimpleKeyAffinityService<>(generator);
            }
        };
    }

...
public enum InfinispanCacheBindingFactory implements Function<BinaryServiceConfiguration, ContextNames.BindInfo> {
    ...
    public ContextNames.BindInfo apply(BinaryServiceConfiguration config) {
        return ContextNames.bindInfoFor(JndiNameFactory.createJndiName(JndiNameFactory.DEFAULT_JNDI_NAMESPACE, InfinispanSubsystemResourceDescription.INSTANCE.getName(), this.name, config.getParentName(), config.getChildName()).getAbsoluteName());
    }
}

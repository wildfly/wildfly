    ...
    JDBCStoreResourceDefinitionRegistrar() {
        super(new Configurator<>() {
            ...
            public ServiceDependency<JdbcStringBasedStoreConfigurationBuilder> resolve(OperationContext context, ModelNode model) throws OperationFailedException {
                ...
                ServiceDependency<TableManipulationConfiguration> table = ServiceDependency.on(TableResourceDefinitionRegistrar.SERVICE_DESCRIPTOR, containerAddress.getLastElement().getValue(), cacheAddress.getLastElement().getValue());

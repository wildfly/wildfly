...
public class CacheContainerServiceConfigurator implements ResourceServiceConfigurator {
    ...
    public ResourceServiceInstaller configure(OperationContext context, ModelNode model) throws OperationFailedException {
        ...
        List<ResourceServiceInstaller> installers = new ArrayList<>(aliases.size() + 2);
        ServiceName targetName = CAPABILITY.getCapabilityServiceName(context.getCurrentAddress());
        installers.add(new BinderServiceInstaller(InfinispanCacheContainerBindingFactory.EMBEDDED.apply(name), targetName));
        ...
        for (String alias : aliases) {
            ...
            installers.add(new BinderServiceInstaller(InfinispanCacheContainerBindingFactory.EMBEDDED.apply(alias), targetName));
        }

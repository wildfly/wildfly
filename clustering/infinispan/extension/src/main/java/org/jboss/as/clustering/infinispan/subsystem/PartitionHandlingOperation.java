...
public enum PartitionHandlingOperation implements RuntimeOperation<AdvancedCache<?, ?>> {
    ...
    PartitionHandlingOperation(String name) {
        this.definition = new SimpleOperationDefinitionBuilder(name, InfinispanSubsystemResourceDefinitionRegistrar.RESOLVER.createChildResolver(PartitionHandlingRuntimeResourceDescription.INSTANCE.getPathElement()))
                .setRuntimeOnly()
                .build();
    }

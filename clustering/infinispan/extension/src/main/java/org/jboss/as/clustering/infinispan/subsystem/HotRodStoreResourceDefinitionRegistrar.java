...
public class HotRodStoreResourceDefinitionRegistrar extends StoreResourceDefinitionRegistrar<HotRodStoreConfiguration, HotRodStoreConfigurationBuilder> {
    ...
    static final HotRodCacheConfigurationAttributeGroup CACHE_ATTRIBUTES = new HotRodCacheConfigurationAttributeGroup(CAPABILITY);
    ...
    HotRodStoreResourceDefinitionRegistrar() {
        super(new Configurator<>() {
            ...
            public ServiceDependency<HotRodStoreConfigurationBuilder> resolve(OperationContext context, ModelNode model) throws OperationFailedException {
                String cacheConfiguration = CACHE_ATTRIBUTES.getCacheAttribute().resolveModelAttribute(context, model).asStringOrNull();
                return CACHE_ATTRIBUTES.getContainerAttribute().resolve(context, model).map(new Function<>() {
                    @Override
    ...
    public ResourceDescriptor.Builder apply(ResourceDescriptor.Builder builder) {
        return CACHE_ATTRIBUTES.apply(super.apply(builder));
    }
}

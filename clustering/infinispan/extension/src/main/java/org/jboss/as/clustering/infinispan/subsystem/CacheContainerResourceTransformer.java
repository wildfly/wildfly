...
public class CacheContainerResourceTransformer implements Consumer<ModelVersion> {
    ...
    CacheContainerResourceTransformer(ResourceTransformationDescriptionBuilder parent) {
        this.builder = parent.addChildResource(CacheContainerResourceDescription.INSTANCE.getPathElement());
    }
    ...
    public void accept(ModelVersion version) {
        ...
        if (InfinispanSubsystemModel.VERSION_15_0_0.requiresTransformation(version)) {
            this.builder.getAttributeBuilder()
                    .setDiscard(DiscardAttributeChecker.DEFAULT_VALUE, CacheContainerResourceDescription.MARSHALLER)
                    .addRejectCheck(new RejectAttributeChecker.SimpleAcceptAttributeChecker(CacheContainerResourceDescription.MARSHALLER.getDefaultValue()), CacheContainerResourceDescription.MARSHALLER)
                    .end();
        }
        ...
        if (!legacyModules.isEmpty()) {
            ...
            }, CacheContainerResourceDescription.MODULES)
            .end();
        }

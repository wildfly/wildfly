...
public class SecurityResourceDefinition extends ComponentResourceDefinition {
    ...
    public ResourceServiceInstaller configure(OperationContext context, ModelNode model) throws OperationFailedException {
        ...
        ServiceDependency<SSLContext> sslContextDependency = (sslContextName != null) ? ServiceDependency.on(CommonServiceDescriptor.SSL_CONTEXT, sslContextName) : ServiceDependency.of(null);

        Supplier<SecurityConfiguration> configurationFactory = new Supplier<>() {
            @Override
            public SecurityConfiguration get() {
                SSLContext sslContext = sslContextDependency.get();
                return new ConfigurationBuilder().security().ssl().sslContext(sslContext).enabled(sslContext != null).security().create();
            }
        };
        return CapabilityServiceInstaller.builder(CAPABILITY, configurationFactory)
                .requires(sslContextDependency)
                .build();
    }
}

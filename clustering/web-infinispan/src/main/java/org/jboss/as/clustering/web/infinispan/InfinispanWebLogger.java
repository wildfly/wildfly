...
interface InfinispanWebLogger extends ClusteringLogger {
    ...
    @Message(id = 10270, value = "Adding missing jvm route entry to web session cache")
    ...
    @Message(id = 10271, value = "%s replication granularity is deprecated.  Falling back to %s granularity instead.")
     ...
     */
    @LogMessage(level = WARN)
    @Message(id = 10272, value = "Problem accessing session [%s]: %s")
     ...
     */
    @LogMessage(level = INFO)
    @Message(id = 10273, value = "Removed stale jvm route entry from web session cache on behalf of member %s")
     ...
     */
    @LogMessage(level = INFO)
    @Message(id = 10274, value = "Updating jvm route entry in web session cache.  old = %s, new = %s")
     ...
     */
    @LogMessage(level = WARN)
    @Message(id = 10275, value = "Possible concurrency problem: Replicated version id %d is less than or equal to in-memory version for session %s")

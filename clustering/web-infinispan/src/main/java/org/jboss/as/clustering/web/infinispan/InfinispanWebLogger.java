...
interface InfinispanWebLogger extends ClusteringLogger {
    ...
    @Message(id = 10250, value = "Adding missing jvm route entry to web session cache")
    ...
    @Message(id = 10251, value = "%s replication granularity is deprecated.  Falling back to %s granularity instead.")
     ...
     */
    @LogMessage(level = WARN)
    @Message(id = 10252, value = "Problem accessing session [%s]: %s")
     ...
     */
    @LogMessage(level = INFO)
    @Message(id = 10253, value = "Removed stale jvm route entry from web session cache on behalf of member %s")
     ...
     */
    @LogMessage(level = INFO)
    @Message(id = 10254, value = "Updating jvm route entry in web session cache.  old = %s, new = %s")
     ...
     */
    @LogMessage(level = WARN)
    @Message(id = 10255, value = "Possible concurrency problem: Replicated version id %d is less than or equal to in-memory version for session %s")

                    ...
                    do {
                        if (group != null) {
                            if (!group.getReaderNames().contains(reader.getName())) {
                                if (occurrences < group.getCardinality().getMinOccurs()) {
                                    throw ClusteringLogger.ROOT_LOGGER.minOccursNotReached(reader, group.getNames(), group.getCardinality());
                                }
                        ...
                        if (group != null) {
                            ...
                            if (occurrences > group.getCardinality().getMaxOccurs().orElse(Integer.MAX_VALUE)) {
                                throw ClusteringLogger.ROOT_LOGGER.maxOccursExceeded(reader, group.getNames(), group.getCardinality());
                            }

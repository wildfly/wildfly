    ...
    class DefaultXMLContent<RC, WC> implements XMLContent<RC, WC> {
        ...
        public void readContent(XMLExtendedStreamReader reader, RC context) throws XMLStreamException {
            ...
            if (reader.hasNext() && reader.nextTag() != XMLStreamConstants.END_ELEMENT) {
                ...
                do {
                    if (!this.group.getReaderNames().contains(reader.getName())) {
                        throw ParseUtils.unexpectedElement(reader, this.group.getReaderNames().stream().map(QName::getLocalPart).collect(Collectors.toSet()));
                    }
                    occurrences += 1;
                    // Validate maxOccurs
                    if (occurrences > maxOccurs) {
                        throw ClusteringLogger.ROOT_LOGGER.maxOccursExceeded(reader, this.group.getNames(), this.group.getCardinality());
                    }
            ...
            if (occurrences < this.group.getCardinality().getMinOccurs()) {
                throw ClusteringLogger.ROOT_LOGGER.minOccursNotReached(reader, this.group.getNames(), this.group.getCardinality());
            }

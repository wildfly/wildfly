...
public interface XMLParticleGroup<RC, WC> extends XMLElementGroup<RC, WC> {
    ...
    static <RC, WC> XMLParticleGroup<RC, WC> empty() {
        return new DefaultXMLParticleGroup<>(Set.of(), XMLCardinality.NONE, new XMLElementReader<>() {
            @Override
    ...
    abstract class AbstractBuilder<RC, WC, T extends XMLParticleGroup<RC, WC>, B extends Builder<RC, WC, T, B>> extends XMLParticle.AbstractBuilder<RC, WC, T, B> implements Builder<RC, WC, T, B>, FeatureFilter {
        ...
        private B addGroup(XMLParticleGroup<RC, WC> group) {
            // Omit empty group
            if (!group.getNames().isEmpty() && this.filter.enables(group)) {

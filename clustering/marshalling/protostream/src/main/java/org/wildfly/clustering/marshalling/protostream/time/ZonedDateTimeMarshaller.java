 ...
 * @author Paul Ferraro
 */
public enum ZonedDateTimeMarshaller implements AutoSizedProtoStreamMarshaller<ZonedDateTime> {
    INSTANCE;
    ...
    private static final int ZONE_INDEX = 8;
    ...
    public ZonedDateTime readFrom(ImmutableSerializationContext context, RawProtoStreamReader reader) throws IOException {
        ...
        while (reading) {
            ...
            switch (index) {
                ...
                case 6:
                case 7:
                    zone = ZoneOffsetMarshaller.INSTANCE.readField(context, reader, index - OFFSET_INDEX, (ZoneOffset) zone);
                    break;
                case ZONE_INDEX:
    ...
    public void writeTo(ImmutableSerializationContext context, RawProtoStreamWriter writer, ZonedDateTime value) throws IOException {
        ...
        if (!zone.equals(DEFAULT_ZONE)) {
            if (zone instanceof ZoneOffset) {
                ZoneOffsetMarshaller.INSTANCE.writeFields(context, writer, OFFSET_INDEX, (ZoneOffset) zone);
            } else {

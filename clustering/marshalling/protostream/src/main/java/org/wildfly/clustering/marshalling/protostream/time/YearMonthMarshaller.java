...
public enum YearMonthMarshaller implements ProtoStreamMarshaller<YearMonth> {
    ...
    private static final YearMonth DEFAULT = YearMonth.of(YearFieldMarshaller.EPOCH, Month.JANUARY);

    private static final int YEAR_INDEX = 1;
    private static final int MONTH_INDEX = YEAR_INDEX + YearFieldMarshaller.INSTANCE.getFields();
    ...
    public YearMonth readFrom(ImmutableSerializationContext context, RawProtoStreamReader reader) throws IOException {
        ...
        while (reading) {
            ...
            if (index >= YEAR_INDEX && index < MONTH_INDEX) {
                result = result.withYear(YearFieldMarshaller.INSTANCE.readField(context, reader, index - YEAR_INDEX, Year.of(result.getYear())).getValue());
    ...
    public void writeTo(ImmutableSerializationContext context, RawProtoStreamWriter writer, YearMonth value) throws IOException {
        ...
        if (year != DEFAULT.getYear()) {
            YearFieldMarshaller.INSTANCE.writeFields(context, writer, YEAR_INDEX, Year.of(year));
        }

...
public enum InetSocketAddressMarshaller implements ProtoStreamMarshaller<InetSocketAddress> {
    ...
    private static final InetSocketAddress DEFAULT = new InetSocketAddress(InetAddress.getLoopbackAddress(), 0);
    private static final int RESOLVED_ADDRESS_INDEX = 1;
    private static final int UNRESOLVED_HOST_INDEX = RESOLVED_ADDRESS_INDEX + InetAddressFieldMarshaller.INSTANCE.getFields();
    private static final int PORT_INDEX = UNRESOLVED_HOST_INDEX + 1;

    @Override
    public InetSocketAddress readFrom(ImmutableSerializationContext context, RawProtoStreamReader reader) throws IOException {
        ...
        while (reading) {
            ...
            if (index >= RESOLVED_ADDRESS_INDEX && index < UNRESOLVED_HOST_INDEX) {
                result = new InetSocketAddress(InetAddressFieldMarshaller.INSTANCE.readField(context, reader, index - RESOLVED_ADDRESS_INDEX, result.getAddress()), result.getPort());
    ...
    @Override
    public void writeTo(ImmutableSerializationContext context, RawProtoStreamWriter writer, InetSocketAddress socketAddress) throws IOException {
        if (socketAddress.isUnresolved()) {
            writer.writeString(UNRESOLVED_HOST_INDEX, socketAddress.getHostName());
        } else {
            InetAddress address = socketAddress.getAddress();
            if (!address.equals(DEFAULT.getAddress())) {
                InetAddressFieldMarshaller.INSTANCE.writeFields(context, writer, RESOLVED_ADDRESS_INDEX, address);
            }
        }

...
public final class VaultSession {
  ...
  protected void validateKeystoreURL() throws Exception {
    ...
    if ( !f.exists()) {
      throw new Exception("Keystore [" + keystoreURL + "] doesn't exist."
          + "\nkeystore could be created: keytool -genkey -alias vault -keyalg RSA -keysize 1024  -keystore " + keystoreURL);
  ...
  /**
   * Add secured attribute to specified vault block. This method can be called only after successful startVaultSession() call. After
   * successful storage the secured attribute information will be displayed at standard output. For silent method @see addSecuredAttribute
   * 
   * @param vaultBlock
   * @param attributeName
   * @param attributeValue
  ...
  /**
   * Returns configuration string for secured attribute. keyAsString parameter can be null. In this case proper value will be calculated.
   * 
   * @param vaultBlock
   * @param attributeName
   * @param keyAsString
  ...
  /**
   * Method to get keystore masked password to use further in configuration. Has to be used after {@link startVaultSession} method.
   * 

...
public class DatabaseConnectionManagerService implements Service<DatabaseConnectionManagerService>, ConnectionManager {
    ...
    private static final long MAX_WAIT_FOR_LOCK = 60000;
    private static final long MAX_WAIT_FOR_UNUSED_CONNECTIONS = 60000;
    private static final long MAX_DELAY_FOR_REPEAER_REQUEST = 300000;
    ...
    */
    ...
    public synchronized void start(StartContext context) throws StartException {
        try {
            if (resolvedConfiguration.hasDefined(DatabaseConnectionResourceDefinition.DATA_SOURCE.getName())) {
                databaseConnectionPool = new DatabaseConnectionPool(resolvedConfiguration.require(DatabaseConnectionResourceDefinition.DATA_SOURCE.getName()).asString());
            } else {
                databaseConnectionPool = new DatabaseConnectionPool(resolvedConfiguration.require(DatabaseConnectionResourceDefinition.DATABASE_MODULE.getName()).asString(),
                        ...
                        MAX_WAIT_FOR_LOCK,
                        MAX_WAIT_FOR_UNUSED_CONNECTIONS,
                        MAX_DELAY_FOR_REPEAER_REQUEST);
            }
        } catch (NamingException e) {
            throw MESSAGES.datasourceLookupException(e,DatabaseConnectionResourceDefinition.DATA_SOURCE.getName());

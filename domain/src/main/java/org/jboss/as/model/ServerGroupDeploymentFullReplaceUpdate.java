...
/**
 * Update used when replacng a deployment in a server group with another deployment.
 *
 * @author John E. Bailey
 * @author Brian Stansberry
 */
public class ServerGroupDeploymentFullReplaceUpdate extends AbstractModelUpdate<ServerGroupElement, ServerDeploymentActionResult> {
    ...
    public ServerGroupDeploymentFullReplaceUpdate(final String uniqueName, final String runtimeName, final byte[] hash, final String replacedDeploymentName) {
        if (uniqueName == null)
    ...
    public ServerGroupDeploymentFullReplaceUpdate getCompensatingUpdate(ServerGroupElement original) {
        ...
        if (toReplace == null)
            return null;
        return new ServerGroupDeploymentFullReplaceUpdate(toReplace.getUniqueName(), toReplace.getRuntimeName(), toReplace.getSha1Hash(), uniqueName);
    }

    @Override
    protected ServerModelDeploymentReplaceUpdate getServerModelUpdate() {
        return new ServerModelDeploymentReplaceUpdate(uniqueName, replacedDeploymentName);
    }
    ...
    protected void applyUpdate(ServerGroupElement serverGroupElement) throws UpdateFailedException {
        ...
        if (deploymentElement == null) {
            throw new UpdateFailedException("Unknown deployment " + uniqueName);
        }

...
/**
 * Applies a JVM modification to a {@link JvmElement} nested in a {@link ServerElement}.
 *
 * @author Brian Stansberry
 */
public class ServerElementJvmUpdate<R> extends AbstractModelUpdate<ServerElement, R> {

    private static final long serialVersionUID = 6228523925797316634L;

    private final AbstractModelUpdate<JvmElement, R> jvmUpdate;

    public static <T> ServerElementJvmUpdate<T> create(final AbstractModelUpdate<JvmElement, T> jvmUpdate) {
        return new ServerElementJvmUpdate<T>(jvmUpdate);
    }

    public ServerElementJvmUpdate(final AbstractModelUpdate<JvmElement, R> jvmUpdate) {
        if (jvmUpdate == null)
    ...
    @Override
    public ServerElementJvmUpdate<?> getCompensatingUpdate(ServerElement original) {

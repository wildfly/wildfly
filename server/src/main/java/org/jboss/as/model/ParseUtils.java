...
public final class ParseUtils {
    ...
    public static List<SchemaLocation> readSchemaLocations(final XMLExtendedStreamReader reader, final int idx) throws XMLStreamException {
        final List<String> values = reader.getListAttributeValue(idx);
        if ((values.size() & 1) != 0) {
            throw invalidAttributeValue(reader, idx);
        }
        final Iterator<String> it = values.iterator();
        final List<SchemaLocation> locationList = new ArrayList<SchemaLocation>();
        while (it.hasNext()) {
            locationList.add(new SchemaLocation(it.next(), it.next()));
        }
        return locationList;
    }

    public static List<NamespacePrefix> readNamespaces(final XMLExtendedStreamReader reader) {
        final List<NamespacePrefix> prefixes;
        final int nsCount = reader.getNamespaceCount();
        if (nsCount > 0) {
            prefixes = new ArrayList<NamespacePrefix>();

...
import static org.jboss.as.mail.extension.ModelKeys.OUTGOING_SOCKET_BINDING_REF;
...
public class Util {
    ...
    private static void addServerConfig(final ModelNode operation, final MailSessionServer server, final String name) {
        operation.get(name).get(OUTGOING_SOCKET_BINDING_REF).set(server.getOutgoingSocketBinding());
    ...
    private static MailSessionServer readServerConfig(final OperationContext operationContext, final ModelNode model) {
        final String socket = model.require(OUTGOING_SOCKET_BINDING_REF).asString();

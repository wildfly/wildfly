...
public class MailSessionService implements Service<Session> {
    ...
    public void start(StartContext startContext) throws StartException {
        log.trace("start...");
        final Session sess = Session.getDefaultInstance(getProperties());
        setAuthentication(sess);
        log.trace("session is: " + sess);
        this.session = sess;
    }
    ...
    private Properties getProperties() throws StartException  {
        ...
        if (config.getSmtpServer() != null) {
            ...
            props.put("mail.smtp.host", socketAddress.getAddress().getHostName());
            props.put("mail.smtp.port", socketAddress.getPort());
        }
        if (config.getImapServer() != null) {
            InetSocketAddress socketAddress = getServerSocketAddress(config.getImapServer());
            props.put("mail.imap.host", socketAddress.getAddress().getHostName());
            props.put("mail.imap.port", socketAddress.getPort());
        }
        if (config.getPop3Server() != null) {
            InetSocketAddress socketAddress = getServerSocketAddress(config.getPop3Server());
            props.put("mail.pop3.host", socketAddress.getAddress().getHostName());
            props.put("mail.pop3.port", socketAddress.getPort());
        }
    ...
    private void setAuthentication(final Session session)  throws StartException {
    ...
    private void setAuthForServer(final Session session, final MailSessionServer server, final String protocol)  throws StartException {
        if (server != null) {
            InetSocketAddress socketAddress = getServerSocketAddress(server);
            Credentials c = server.getCredentials();
            URLName urlName = new URLName(protocol, socketAddress.getAddress().getHostName(), socketAddress.getPort(), "", c != null ? c.getUsername() : null, c != null ? c.getPassword() : null);
            if (c != null) {

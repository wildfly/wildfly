 ...
 * @author Jaikiran Pai
 */
class LocalOutboundConnectionWriteHandler extends AbstractWriteAttributeHandler<Void> {
    ...
    private void applyModelToRuntime(OperationContext context, ModelNode operation, String attributeName, ModelNode model) throws OperationFailedException {
        ...
        if (sc != null) {
            LocalOutboundConnectionService outboundConnectionService = LocalOutboundConnectionService.class.cast(sc.getValue());
        ...
        LocalOutboundConnectionAdd.INSTANCE.installRuntimeService(context, model, null);
    }
}

...
class EJBClientDescriptor13Parser implements XMLElementReader<EJBClientDescriptorMetaData> {
    ...
    private EJBClientDescriptor13Parser() {
    }
    ...
    enum Element {
        ...
        static {
            ...
            elementsMap.put(new QName(NAMESPACE_1_3,"profile"), Element.PROFILE);
    ...
    private void parseProfile(final XMLExtendedStreamReader reader, final EJBClientDescriptorMetaData ejbClientDescriptorMetaData) throws XMLStreamException {
        final Set<Attribute> required = EnumSet.of(Attribute.NAME);
        final int count = reader.getAttributeCount();
        ...
        for (int i = 0; i < count; i++) {
            ...
            switch (attribute) {
                case NAME:
                    profileName = reader.getAttributeValue(i).trim();
                    break;
                default:
                    unexpectedContent(reader);
            }

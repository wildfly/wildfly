...
public abstract class AbstractSetupTask implements ServerSetupTask {
    public static ModelNode clearAttribute(String address, String attributeName) {
        ModelNode op = Operations.createOperation("write-attribute", Operations.createAddress(SUBSYSTEM, address));
        op.get("name").set(attributeName);
        return op;
    }

    public static ModelNode writeAttribute(String subsystem, String name, String value) {
        return writeAttribute(Operations.createAddress(SUBSYSTEM, subsystem), name, value);
    }

    public static ModelNode writeAttribute(ModelNode address, String name, String value) {
        return Operations.createWriteAttributeOperation(address, name, value);
    }

    public static void executeOp(final ManagementClient client, final ModelNode op) throws IOException {
        executeOp(client.getControllerClient(), Operation.Factory.create(op));
    }

    public static void executeOp(final ModelControllerClient client, final Operation op) throws IOException {
    ...
    public static ModelNode executeRead(final ManagementClient managementClient, ModelNode address) throws IOException {

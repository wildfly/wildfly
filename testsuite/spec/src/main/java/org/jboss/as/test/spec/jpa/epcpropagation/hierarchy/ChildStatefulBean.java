...
public class ChildStatefulBean extends BeanParent {
    ...
    public void testPropagation() throws Exception {
        ...
        if(!simpleStatefulBean.contains(b)){
            throw new AssertionError("simple stateful bean does not contain " + b);
        }
        if(!injectedSimpleStatefulBean.contains(b)){
            throw new AssertionError("injected simple stateful bean does not contain " + b);
        }
        if(!entityManager.contains(b)){
            throw new AssertionError("EM does not contain " + b);
        }
    }
}

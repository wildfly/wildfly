...
@ServerSetup({InMemoryCollectorSetupTask.class, MicrometerSetupTask.class})
public class FaultToleranceOpenTelemetryIntegrationTestCase {
    ...
    public void test() throws Exception {
        ...
        collector.assertMetrics(metrics -> {
            Optional<SimpleMetric> simpleMetric;

            // First verify total invocation count for the method + value returned + fallback applied
            simpleMetric = metrics.stream().filter(metric -> metric.name().equals("ft.invocations.total")).findFirst();

...
public class StartWorkersInSuspendedModeTestCase extends AbstractClusteringTestCase {
    ...
    static class ServerSetupTask extends CLIServerSetupTask {
        public ServerSetupTask() {
            ...
            this.builder
               .node(NODE_1)
               .setup("/subsystem=modcluster/mod-cluster-config=configuration:write-attribute(name=advertise,value=false)")
               ...
               .setup("/subsystem=modcluster/mod-cluster-config=configuration:list-add(name=proxies,value=proxy1)")
               .teardown("/subsystem=modcluster/mod-cluster-config=configuration:list-remove(name=proxies,value=proxy1)")
               .teardown("/socket-binding-group=standard-sockets/remote-destination-outbound-socket-binding=proxy1:remove()")
               .teardown("/subsystem=modcluster/mod-cluster-config=configuration:write-attribute(name=advertise,value=true)")
            ;
        }
    }

...
public class ElytronSSOServerSetupTask extends CLIServerSetupTask {
    public ElytronSSOServerSetupTask() {
        this.builder.node(AbstractClusteringTestCase.TWO_NODES)
                ...
                .setup("/subsystem=undertow/application-security-domain=other:add(http-authentication-factory=sso)")
                .setup("/subsystem=elytron/key-store=sso:add(path=sso.keystore, relative-to=jboss.server.config.dir, credential-reference={clear-text=password}, type=PKCS12)")
                ...
                .teardown("/subsystem=undertow/application-security-domain=other:remove()")
                .teardown("/subsystem=elytron/http-authentication-factory=sso:remove()")

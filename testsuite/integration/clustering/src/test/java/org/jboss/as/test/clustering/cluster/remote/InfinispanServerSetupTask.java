...
package org.jboss.as.test.clustering.cluster.remote;
...
public class InfinispanServerSetupTask extends ManagementServerSetupTask {
    protected InfinispanServerSetupTask(Set<String> containers, String name, String module) {
        super(containers, createContainerConfigurationBuilder()
                .setupScript(createScriptBuilder()
                    .startBatch()
                        ...
                        .add("/subsystem=infinispan/remote-cache-container=%s:add(default-remote-cluster=infinispan-server-cluster, tcp-keep-alive=true, marshaller=PROTOSTREAM, modules=%s, properties={infinispan.client.hotrod.auth_username=%s, infinispan.client.hotrod.auth_password=%s}, statistics-enabled=true)", name, List.of(module), INFINISPAN_APPLICATION_USER, INFINISPAN_APPLICATION_PASSWORD)
                        .add("/subsystem=infinispan/remote-cache-container=%s/remote-cluster=infinispan-server-cluster:add(socket-bindings=[infinispan-server])", name)
                    .endBatch()
                    .build())
                .tearDownScript(createScriptBuilder()
                    .startBatch()
                        .add("/subsystem=infinispan/remote-cache-container=%s:remove", name)

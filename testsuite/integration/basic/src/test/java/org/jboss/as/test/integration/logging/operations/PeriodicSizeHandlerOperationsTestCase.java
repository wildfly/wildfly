...
@RunAsClient
public class PeriodicSizeHandlerOperationsTestCase extends AbstractLoggingOperationsTestCase {
    private static final Logger log = Logger.getLogger(PeriodicSizeHandlerOperationsTestCase.class.getName());
    private static final String FILE_NAME = "periodic-size-rotating-handler.log";
    private static final String HANDLER_NAME = PeriodicSizeHandlerOperationsTestCase.class.getSimpleName();
    private static final ModelNode HANDLER_ADDRESS = createPeriodicSizeRotatingFileHandlerAddress(HANDLER_NAME).toModelNode();
    ...
    public void testRemove() throws Exception {
        ...
        ModelNode address = createPeriodicSizeRotatingFileHandlerAddress(PROFILE_NAME, HANDLER_NAME).toModelNode();
        addHandler(address);
    ...
    @Test
    public void testPeriodicSizeHandler() throws Exception {
        testPeriodicSizeHandler(null);

        // Create the profile
        final ModelNode addOp = Operations.createAddOperation(PROFILE_ADDRESS);
        executeOperation(addOp);

        testPeriodicSizeHandler(PROFILE_NAME);
    }

    private void testPeriodicSizeHandler(final String profileName) throws Exception {
        ModelNode address = createPeriodicSizeRotatingFileHandlerAddress(profileName, HANDLER_NAME).toModelNode();

...
public class SecurityContextImplLoginTestCase {
    ...
    public void test() throws Exception{
        try (CloseableHttpClient client = HttpClientBuilder.create().build()) {
            String cURL = "http://127.0.0.1:8080/" + DEPLOYMENT + "/main";
            HttpResponse response = client.execute(new HttpGet(cURL));
    ...
    public void test2() throws Exception{
        try (CloseableHttpClient client = HttpClientBuilder.create().build()) {
            String cURL = "http://localhost:8080/" + DEPLOYMENT + "/main/?login=true";
            HttpResponse response = client.execute(new HttpGet(cURL));
            Assert.assertEquals("For more info see ELYWEB-133. " ,200, response.getStatusLine().getStatusCode());
        }
    }

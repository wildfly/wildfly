...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
                xmlns:ispn="urn:jboss:domain:infinispan:2.0" version="1.0">
	...
	<xsl:variable name="newCacheDefinition">
		<cache-container name="testDbPersistence"
			default-cache="jdbc-cache">
			<transport lock-timeout="60000" />
			<xsl:element name="{$cacheType}-cache"
				use-attribute-sets="cacheAttributes">
				<xsl:element name="{$cacheLoader}-jdbc-store"
					use-attribute-sets="cacheLoaderAttributes">
					<xsl:choose>
	...
	<xsl:template match="//ispn:subsystem/ispn:cache-container[@name='web']">

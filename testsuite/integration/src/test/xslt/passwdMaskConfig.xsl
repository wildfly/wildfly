<?xml version="1.0" encoding="UTF-8"?>
<!-- Edited by XMLSpy® -->
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">

    <xsl:output indent="yes"/>

    <xsl:variable name="jboss" select="'urn:jboss:domain:'"/>
    <xsl:variable name="datasources" select="'urn:jboss:domain:datasources'"/>

    <xsl:param name="keystore"/>
    <xsl:param name="encFileDir"/>

    <xsl:template match="//*[local-name()='extensions' and starts-with(namespace-uri(), $jboss)]"
                         priority="100">
        <xsl:copy>
            <xsl:apply-templates select="node()"/>
        </xsl:copy>
        <xsl:element name="vault" namespace="{namespace-uri()}">
            <xsl:element name="vault-option" namespace="{namespace-uri()}">
                <xsl:attribute name="name">KEYSTORE_URL</xsl:attribute>
                <xsl:attribute name="value"><xsl:value-of select="$keystore"/></xsl:attribute>
            </xsl:element>
            <xsl:element name="vault-option" namespace="{namespace-uri()}">
                <xsl:attribute name="name">KEYSTORE_PASSWORD</xsl:attribute>
                <!-- clear text password for keystore is secretsecret -->
                <xsl:attribute name="value">MASK-1CY3kNqbYVLo5njerVXKG7</xsl:attribute>
            </xsl:element>
            <xsl:element name="vault-option" namespace="{namespace-uri()}">
                <xsl:attribute name="name">KEYSTORE_ALIAS</xsl:attribute>
                <xsl:attribute name="value">vault</xsl:attribute>
            </xsl:element>
            <xsl:element name="vault-option" namespace="{namespace-uri()}">
                <xsl:attribute name="name">SALT</xsl:attribute>
                <xsl:attribute name="value">12345678</xsl:attribute>
            </xsl:element>
            <xsl:element name="vault-option" namespace="{namespace-uri()}">
                <xsl:attribute name="name">ITERATION_COUNT</xsl:attribute>
                <xsl:attribute name="value">50</xsl:attribute>
            </xsl:element>
            <xsl:element name="vault-option" namespace="{namespace-uri()}">
                <xsl:attribute name="name">ENC_FILE_DIR</xsl:attribute>
                <xsl:attribute name="value"><xsl:value-of select="$encFileDir"/></xsl:attribute>
            </xsl:element>
        </xsl:element>
    </xsl:template>

    <xsl:template match="//*[local-name()='subsystem' and starts-with(namespace-uri(), jboss)]
                          /*[local-name()='datasources' and starts-with(namespace-uri(), $datasources)]"
                         priority="100">
        <xsl:copy>
            <xsl:element name="datasource" namespace="{namespace-uri()}">
                <xsl:attribute name="jndi-name">java:jboss/datasources/MaskedDS</xsl:attribute>
                <xsl:attribute name="use-java-context">true</xsl:attribute>
                <xsl:attribute name="enabled">true</xsl:attribute>
                <xsl:attribute name="pool-name">MaskedDS</xsl:attribute>
                <xsl:element name="connection-url" namespace="{namespace-uri()}">jdbc:h2:mem:masked;DB_CLOSE_DELAY=-1</xsl:element>
                <xsl:element name="driver" namespace="{namespace-uri()}">h2</xsl:element>
                <xsl:element name="security" namespace="{namespace-uri()}">
                    <xsl:element name="user-name" namespace="{namespace-uri()}">sa</xsl:element>
                    <!-- clear text password is sa -->
                    <xsl:element name="password" namespace="{namespace-uri()}">${VAULT::ds_ExampleDS::password::1}</xsl:element>
                </xsl:element>
            </xsl:element>
            <xsl:apply-templates select="node()"/>
        </xsl:copy>
    </xsl:template>

    <xsl:template match="@*|node()" priority="1">
        <xsl:copy>
            <xsl:apply-templates select="@*|node()"/>
        </xsl:copy>
    </xsl:template>

</xsl:stylesheet>

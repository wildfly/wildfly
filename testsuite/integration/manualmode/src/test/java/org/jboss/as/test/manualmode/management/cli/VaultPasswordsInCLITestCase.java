...
import org.jboss.as.arquillian.container.Authentication.CallbackHandler;
...
public class VaultPasswordsInCLITestCase {
    ...
    public static final File VAULT_KEYSTORE_FILE = new File(WORK_DIR, "vault.keystore");
    ...
    public void testWrongVaultPassword() throws InterruptedException, IOException {
        ...
        assertTrue("Password should be wrong", cliOutput.contains("Keystore was tampered with, or password was incorrect"));
    ...
    public void testRightVaultPassword() throws InterruptedException, IOException {
        ...
        assertTrue("Password should be right and can access native interface.", cliOutput.contains("\"outcome\" => \"success\""));
    ...
    /**
     * Creates keystore for vault usage by keytool
     * 
     * @throws Exception
     */
    private void createKeystoreForVault() throws Exception {
        Process p = Runtime.getRuntime().exec(
                new String[] { "keytool", "-genseckey", "-alias", ALIAS_NAME, "-storetype", "jceks", "-keyalg", "AES", "-keysize", "128",
                        "-keystore", VAULT_KEYSTORE_FILE.getAbsolutePath(), "-storepass", VAULT_KEYSTORE_PASS, "-dname",
                        "cn=JBoss Security,ou=JBoss Division,o=RedHatInc,l=Raleigh,st=NC,c=US", "-keypass", VAULT_KEYSTORE_PASS }, null);
    ...
    private void createAndInitializeVault() throws Exception {
        ...
        // create keystore and inilialize vault
        createKeystoreForVault();
        ...
        String wrongVaultPasswordString = nonInteractiveSession.addSecuredAttribute(wrongBlock, attributeName,
                wrongPassword.toCharArray());

        // create jboss-cli configuration file with ssl and vaulted
        // passwords
        FileUtils.write(RIGHT_VAULT_PASSWORD_FILE, Utils.propertiesReplacer(JBOSS_CLI_FILE, CLIENT_KEYSTORE_FILE,
                CLIENT_TRUSTSTORE_FILE, vaultPasswordString, VAULT_CONFIG_FILE));
        FileUtils.write(WRONG_VAULT_PASSWORD_FILE, Utils.propertiesReplacer(JBOSS_CLI_FILE, CLIENT_KEYSTORE_FILE,
                CLIENT_TRUSTSTORE_FILE, wrongVaultPasswordString, VAULT_CONFIG_FILE));

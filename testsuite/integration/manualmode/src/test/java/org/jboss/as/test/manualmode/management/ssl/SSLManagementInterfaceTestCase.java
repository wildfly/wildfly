...
/**
 * Testing https connection to management web console with configured SSL. HTTP
 * client uses trustore with accepted server's certificate.
 * 
 * @author Filip Bogyai
 */
...
public class SSLManagementInterfaceTestCase {
	...
	public void testHTTPS() throws Exception {
		...
		DefaultHttpClient httpClient = SSLTruststoreUtil.getHttpClientWithTrustStore(TRUSTSTORE_FILE, TRUSTSTORE_PASSWORD);
		final UsernamePasswordCredentials credentials = new UsernamePasswordCredentials(ADMIN, ADMIN_PASS);
		httpClient.getCredentialsProvider().setCredentials(new AuthScope(AuthScope.ANY), credentials);
	...
	private void serverSetup(ManagementClient managementClient) throws Exception {
		...
		String managementUser = ADMIN + "="
				+ Utils.hashMD5(ADMIN + ":" + MANAGEMENT_REALM + ":" + ADMIN_PASS, Coding.HEX);
		FileUtils.writeStringToFile(MGMT_USERS_FILE, managementUser);

		// add new ManagementConsoleRealm with SSL keystore
		ModelNode operation = createOpNode("core-service=management/security-realm=ManagementConsoleRealm",
				ModelDescriptionConstants.ADD);
		managementClient.getControllerClient().execute(operation);

		operation = createOpNode(
				"core-service=management/security-realm=ManagementConsoleRealm/authentication=properties",
				ModelDescriptionConstants.ADD);
		operation.get(PATH).set(MGMT_USERS_FILE.getAbsolutePath());
		managementClient.getControllerClient().execute(operation);

		operation = createOpNode("core-service=management/security-realm=ManagementConsoleRealm/server-identity=ssl",
				ModelDescriptionConstants.ADD);
	...
	private void serverTearDown(ManagementClient managementClient) throws Exception {
		...
		operation = createOpNode("core-service=management/security-realm=ManagementConsoleRealm",

...
import org.jboss.as.test.integration.osgi.FrameworkManagement;
...
public class DeferredResolveTestCase {
    ...
    private static final String DEFERRED_GOOD_BUNDLE = "deferred-good-bundle.jar";
    private static final String DEFERRED_BAD_BUNDLE = "deferred-bad-bundle.jar";
    private static final String DEFERRED_BUNDLE_A = "deferred-bundle-a.jar";
    private static final String DEFERRED_BUNDLE_A_EAR = "deferred-bundle-a.ear";
    ...
    public void testResolveAPICall() throws Exception {
        InputStream input = deployer.getDeployment(DEFERRED_GOOD_BUNDLE);
        Bundle bundle = context.installBundle(DEFERRED_GOOD_BUNDLE, input);
        try {
    ...
    public void testFailOnDeploy() throws Exception {
        try {
            deployer.deploy(DEFERRED_BAD_BUNDLE);
    ...
    public void testFailAPICall() throws Exception {
        InputStream input = deployer.getDeployment(DEFERRED_BAD_BUNDLE);
        Bundle bundle = context.installBundle(DEFERRED_BAD_BUNDLE, input);
        try {
            Assert.assertEquals("Bundle INSTALLED", Bundle.INSTALLED, bundle.getState());
            try {
                bundle.start();
    ...
    @Test
    public void testUnresolvedEar() throws Exception {
        deployer.deploy(DEFERRED_BUNDLE_A_EAR);
        try {
            Bundle bundleA = packageAdmin.getBundles(DEFERRED_BUNDLE_A, null)[0];
            Assert.assertEquals("Bundle INSTALLED", Bundle.INSTALLED, bundleA.getState());
            deployer.deploy(DEFERRED_BUNDLE_B);
            ...
            deployer.undeploy(DEFERRED_BUNDLE_A_EAR);
        }
    }

    @Deployment(name = DEFERRED_GOOD_BUNDLE, managed = false, testable = false)
    public static Archive<?> getGoodBundle() {
        final JavaArchive archive = ShrinkWrap.create(JavaArchive.class, DEFERRED_GOOD_BUNDLE);
        archive.setManifest(new Asset() {
            @Override
            public InputStream openStream() {
                OSGiManifestBuilder builder = OSGiManifestBuilder.newInstance();
    ...
    @Deployment(name = DEFERRED_BAD_BUNDLE, managed = false, testable = false)
    public static Archive<?> getBadBundle() {
        final JavaArchive archive = ShrinkWrap.create(JavaArchive.class, DEFERRED_BAD_BUNDLE);
    ...
    public static Archive<?> getSimpleEar() {
        ...
        ear.addAsModule(getBundleA());
        ear.addAsModule(getBundleB());
        return ear;
    }

    @Deployment(name = DEFERRED_BUNDLE_A_EAR, managed = false, testable = false)
    public static Archive<?> getDeferredBundleEar() {
        final EnterpriseArchive ear = ShrinkWrap.create(EnterpriseArchive.class, DEFERRED_BUNDLE_A_EAR);
        ear.addAsModule(getBundleA());
        return ear;
    }
}

...
public class JBossOSGiFrameworkFactoryTestCase extends EmbeddedOSGiFrameworkLauncherTestBase {
    ...
    public void testFrameworkFactory() throws Exception {
        ...
        final StringBuffer changeResult = new StringBuffer();
        BundleListener listener = new BundleListener() {
            @Override
            public void bundleChanged(BundleEvent event) {
                changeResult.append(event.getType());
                latch.countDown();
            }
        };
        ...
        try {
            ...
            Assert.assertEquals("", changeResult.toString());
            b.stop();
            latch.await();
            Assert.assertEquals("4", changeResult.toString());
        } finally {

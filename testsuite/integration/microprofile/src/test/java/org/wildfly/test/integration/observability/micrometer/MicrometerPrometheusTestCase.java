...
@ServerSetup({MicrometerSetupTask.class, StabilityServerSetupSnapshotRestoreTasks.Preview.class})
@DockerRequired(AssumptionViolatedException.class)
@RunAsClient
public class MicrometerPrometheusTestCase {
    private static final int REQUEST_COUNT = 5;
    private final ModelNode prometheusRegistryAddress = Operations.createAddress(SUBSYSTEM, "micrometer", "registry", "prometheus");
    private final String prometheusContext = "/prometheus";
    ...
    private void addPrometheusRegistry() throws Exception {
        ModelNode addOperation = Operations.createAddOperation(prometheusRegistryAddress);
        addOperation.get("context").set(prometheusContext);
    ...
    private void setPrometheusSecurity(boolean enabled) throws Exception {
        executeOp(managementClient,
                Operations.createWriteAttributeOperation(prometheusRegistryAddress, "security-enabled",
                        enabled));
        ServerReload.reloadIfRequired(managementClient);
    }
        ...
        try (CloseableHttpClient client = HttpClients.createDefault()) {
            ...
            CloseableHttpResponse resp = client.execute(
                    new HttpGet("http://" + managementClient.getMgmtAddress() + ":" +
                            managementClient.getMgmtPort() + prometheusContext), hcContext);

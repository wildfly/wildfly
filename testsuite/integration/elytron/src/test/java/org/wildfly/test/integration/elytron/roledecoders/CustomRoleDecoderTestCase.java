...
/**
 * Test case for Elytron Custom Role Decoder.
 *
 * @author olukas
...
@RunAsClient
@ServerSetup({CustomRoleDecoderTestCase.SetupTask.class})
public class CustomRoleDecoderTestCase {
    ...
    private static WebArchive deployment(String name) {
        ...
        war.addAsWebInfResource(CustomRoleDecoderTestCase.class.getPackage(), "simple-role-decoder-web.xml", "web.xml");
    ...
    static class SetupTask extends AbstractElytronSetupTask {
        ...
        public void setup(ManagementClient mc, String string) throws Exception {
            try (CLIWrapper cli = new CLIWrapper(true)) {
                File moduleJar = JarUtils.createJarFile("testJar", CustomRoleDecoderImpl.class);
                cli.sendLine("module add --name=" + CUSTOM_ROLE_DECODER_MODULE_NAME
                ...
                cli.sendLine(String.format(
                    "/subsystem=elytron/custom-role-decoder=%s:add(class-name=%s, module=%s, configuration={roleAttr=%s})",
                    DECODE_FROM_ROLE_ATTRIBUTE_A, CustomRoleDecoderImpl.class.getName(), CUSTOM_ROLE_DECODER_MODULE_NAME,
                    ROLE_ATTRIBUTE_A));
                cli.sendLine(String.format(
                    "/subsystem=elytron/custom-role-decoder=%s:add(class-name=%s, module=%s, configuration={roleAttr=%s})",
                    DECODE_FROM_ROLE_ATTRIBUTE_B, CustomRoleDecoderImpl.class.getName(), CUSTOM_ROLE_DECODER_MODULE_NAME,
                    ROLE_ATTRIBUTE_B));
        ...
        public void tearDown(ManagementClient mc, String string) throws Exception {
            ...
            try (CLIWrapper cli = new CLIWrapper(true)) {
                cli.sendLine(String.format("/subsystem=elytron/custom-role-decoder=%s:remove()", DECODE_FROM_ROLE_ATTRIBUTE_B));
                cli.sendLine(String.format("/subsystem=elytron/custom-role-decoder=%s:remove()", DECODE_FROM_ROLE_ATTRIBUTE_A));
            }

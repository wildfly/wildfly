...
/**
 * Smoke test for Elytron Custom Modifiable Realm. It tests only basic functionality of Custom Modifiable Realm. <br>
 *
...
@RunAsClient
@ServerSetup({ CustomModifiableRealmTestCase.SetupTask.class })
public class CustomModifiableRealmTestCase extends AbstractCustomRealmTestCase {

    private static final String CUSTOM_MODIFIABLE_REALM_MODULE_NAME = "org.jboss.custommodifiablerealmimpl";

    static class SetupTask implements ServerSetupTask {

        private static final String CUSTOM_REALM_NAME = "simpleCustomModifiableRealm";
        private static final String CUSTOM_REALM_RELATED_CONFIGURATION_NAME = "elytronCustomModifiableRealmRelatedConfiguration";
        ...
        public void setup(ManagementClient mc, String string) throws Exception {
            try (CLIWrapper cli = new CLIWrapper(true)) {
                ...
                File moduleJar = JarUtils.createJarFile("testJar", CustomModifiableRealmImpl.class);
                cli.sendLine("module add --name=" + CUSTOM_MODIFIABLE_REALM_MODULE_NAME
                ...
                cli.sendLine(String.format(
                    "/subsystem=elytron/custom-modifiable-realm=%s:add(class-name=%s, module=%s, configuration={%s})",
                    CUSTOM_REALM_NAME, CustomModifiableRealmImpl.class.getName(), CUSTOM_MODIFIABLE_REALM_MODULE_NAME,
        ...
        public void tearDown(ManagementClient mc, String string) throws Exception {
            try (CLIWrapper cli = new CLIWrapper(true)) {
                ...
                cli.sendLine(String.format("/subsystem=elytron/custom-modifiable-realm=%s:remove()", CUSTOM_REALM_NAME));
            }
            ServerReload.reloadIfRequired(mc.getControllerClient());
        }
    }

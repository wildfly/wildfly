...
/**
 * Test case for 'custom-principal-transformer' Elytron subsystem resource.
 *
 * @author olukas
 */
@RunWith(Arquillian.class)
@RunAsClient
@ServerSetup({CustomPrincipalTransformerTestCase.SetupTask.class})
public class CustomPrincipalTransformerTestCase {

    private static final String CUSTOM_PRINCIPAL_TRANSFORMER_MODULE_NAME = "org.jboss.customprincipaltransformerimpl";
    private static final String DEP_SECURITY_DOMAIN = "custom-principal-transformer-domain";
    ...
    public static WebArchive securityDomainWithMappingModuleDeployment() {
        ...
        war.addAsWebInfResource(CustomPrincipalTransformerTestCase.class.getPackage(), "principal-transformer-web.xml", "web.xml");
        war.addAsWebInfResource(Utils.getJBossWebXmlAsset(DEP_SECURITY_DOMAIN), "jboss-web.xml");
        return war;
    }

    /**
     * Test whether name of any user passed to security domain is transformed into custom defined in used
     * 'custom-principal-transformer'. Test also checks that authentication passes for correct password. It also checks that
    ...
    /**
     * Test checks that authentication fails for incorrect password if 'custom-principal-transformer' is used.
     */
    @Test
    @OperateOnDeployment(DEP_SECURITY_DOMAIN)
    ...
    /**
     * Test that even existing user name is transformed into value defined in used 'custom-principal-transformer'. It checks
     * that if password of existing user is passed to authentication, than authentication fails.
     */
    @Test
    @OperateOnDeployment(DEP_SECURITY_DOMAIN)
    ...
    /**
     * Test that even existing user name is transformed into value defined in used 'custom-principal-transformer'. It checks
     * that if password of transformed user is passed to authentication, than authentication passes. It also checks that correct
     * role is assigned to transformed user.
     */
    @Test
    @OperateOnDeployment(DEP_SECURITY_DOMAIN)
    ...
    static class SetupTask implements ServerSetupTask {
        ...
        public void setup(ManagementClient managementClient, String containerId) throws Exception {
            try (CLIWrapper cli = new CLIWrapper(true)) {
                File moduleJar = JarUtils.createJarFile("testJar", CustomPrincipalTransformerImpl.class);
                cli.sendLine("module add --name=" + CUSTOM_PRINCIPAL_TRANSFORMER_MODULE_NAME
                ...
                cli.sendLine(String.format(
                    "/subsystem=elytron/custom-principal-transformer=%s:add(class-name=%s, module=%s, configuration={transformTo=%s})",
                    PRINCIPAL_TRANSFORMER, CustomPrincipalTransformerImpl.class.getName(),
                    CUSTOM_PRINCIPAL_TRANSFORMER_MODULE_NAME, USER1));
                cli.sendLine(String.format(
        ...
        public void tearDown(ManagementClient managementClient, String containerId) throws Exception {
            try (CLIWrapper cli = new CLIWrapper(true)) {
                ...
                cli.sendLine(String.format("/subsystem=elytron/custom-principal-transformer=%s:remove()", PRINCIPAL_TRANSFORMER));
                domain.remove(cli);
            }
            ServerReload.reloadIfRequired(managementClient.getControllerClient());
        }

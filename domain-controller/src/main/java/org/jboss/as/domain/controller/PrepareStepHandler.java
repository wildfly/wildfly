...
public class PrepareStepHandler  implements NewStepHandler {
    ...
    PrepareStepHandler(final String localHostName) {
    ...
    public void execute(NewOperationContext context, ModelNode operation) throws OperationFailedException {
        ...
        if (operation.hasDefined(OPERATION_HEADERS) && operation.get(OPERATION_HEADERS, EXECUTE_FOR_COORDINATOR).asBoolean(false)) {
            executeForCoordinator(context, operation);
        } else {
            executeAsCoordinator(context, operation);
        }
    }

    private void executeAsCoordinator(NewOperationContext context, ModelNode operation) throws OperationFailedException {
        // TODO
//        1) Determine routing
...
//        8) Add a result creation step -- this one actually writes the response, triggers failed or succeeded
    ...
    private void executeForCoordinator(NewOperationContext context, ModelNode operation) throws OperationFailedException {

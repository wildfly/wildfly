...
/**
 * Client interface via which a domain controller interacts with a host controller.
 * Each host controller is registered with the domain controller and provides the
 * domain controller an object implementing interface to communicate with the client.
 *
 *  @author John Bailey
 */
public interface HostControllerClient {
    ...
    /**
     * Get the current state of the host controller's {@link HostModel}.
     *
    ...
    /**
     * Gets a list of all servers known to the host controller, along with their current

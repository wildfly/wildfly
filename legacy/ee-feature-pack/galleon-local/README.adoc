...
This module is a dependency of the `wildfly-legacy-ee-feature-pack` maven module. Its content and dependency entries are used by the build of that module when it creates its feature pack.
...
* Ideally, the `wildfly-legacy-ee-feature-pack` maven module would depend on and unzip these zip artifacts and consume their content that way. But, at least at the time this document was written, it doesn't do this and instead just copies in source from this maven module.
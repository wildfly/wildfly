...
public class IIOPSubsystemAdd extends AbstractBoottimeAddStepHandler {
    ...
    protected void launchServices(final OperationContext context, final ModelNode model) throws OperationFailedException {
        ...
        final Consumer<CorbaORBService> serviceConsumer = builder.provides(org.wildfly.iiop.openjdk.IIOPRootDefinition.IIOP_CAPABILITY);
        Supplier<ExecutorService> executorServiceSupplier = Services.requireServerExecutor(builder);
        Supplier<SocketBinding> iiopSocketBindingSupplier =  () -> null;
        Supplier<SocketBinding> iiopSSLSocketBindingSupplier = () -> null;
        ...
        if (socketBinding != null) {
            if (!serverRequiresSsl) {
                ServiceName serviceName = context.getCapabilityServiceName(SOCKET_BINDING_CAPABILITY_NAME, socketBinding, SocketBinding.class);
                iiopSocketBindingSupplier = builder.requires(serviceName);
            } else {
        ...
        if(sslSocketBinding != null) {
            ServiceName serviceName = context.getCapabilityServiceName(SOCKET_BINDING_CAPABILITY_NAME, sslSocketBinding, SocketBinding.class);
            iiopSSLSocketBindingSupplier = builder.requires(serviceName);
        }
        ...
        builder.addAliases(CorbaORBService.SERVICE_NAME).setInitialMode(ServiceController.Mode.ACTIVE);

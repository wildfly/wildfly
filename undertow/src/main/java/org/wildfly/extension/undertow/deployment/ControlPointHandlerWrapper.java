 ...
 * If request was allowed by the {@link ControlPoint}, request completion is signalled via an {@link ExchangeCompletionListener}.
    ...
    public HttpHandler wrap(HttpHandler handler) {
        ...
        return new HttpHandler() {
            ...
            public void handleRequest(HttpServerExchange exchange) throws Exception {
                ...
                if (accepted || ControlPointHandlerWrapper.this.allowSuspendedRequest(exchange)) {
                    if (accepted) {
                    ...
                    LOGGER.debugf("Rejecting request due to server suspension: %s", exchange.getRequestURI());

...
public class ExternalTldParsingDeploymentProcessor implements DeploymentUnitProcessor {
    ...
    private TldMetaData parseTLD(Resource tld)
        ...
        try {
            ...
            throw new DeploymentUnitProcessingException(UndertowMessages.MESSAGES.failToParseXMLDescriptor(tld.getName(), e.getLocation().getLineNumber(),
                    e.getLocation().getColumnNumber()), e);
        } catch (IOException e) {
            throw new DeploymentUnitProcessingException(UndertowMessages.MESSAGES.failToParseXMLDescriptor(tld.getName()), e);
        } finally {
            try {
                if (is != null) {
                    is.close();

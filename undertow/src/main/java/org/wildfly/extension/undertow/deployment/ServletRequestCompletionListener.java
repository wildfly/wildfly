...
public class ServletRequestCompletionListener implements ServletRequestListener, UnaryOperator<DeploymentInfo> {
    ...
    static class HttpServerExchangeSessionManager implements SessionManager {
        ...
        public Session createSession(HttpServerExchange exchange, SessionConfig config) {
            return !exchange.isComplete() ? this.decorate(exchange, this.manager.createSession(exchange, config)) : new ExchangeCompletedSession(null);
        }
        ...
        private Session decorate(HttpServerExchange exchange, Session session) {
            ...
            exchange.addExchangeCompleteListener(new ExchangeCompletionListener() {
                ...
                public void exchangeEvent(HttpServerExchange exchange, NextListener next) {
                    try {
                        // Trigger request completion logic for the Session on exchange completion
                        session.requestDone(exchange);
                        // Neutralise Undertow's post-response handling of Session
                        Session detached = session.getSessionManager().getSession(session.getId());
                        reference.set(new ExchangeCompletedSession(detached));
                    } finally {
                        next.proceed();
                    }
                }
    ...
    static class ExchangeCompletedSession implements Session {
        ...
        ExchangeCompletedSession(Session session) {
            this.session = session;
        }

        @Override
        public SessionManager getSessionManager() {
            return (this.session != null) ? this.session.getSessionManager() : null;
        }
        ...
        @Override
        public void requestDone(HttpServerExchange exchange) {
            // Do nothing - exchange is already complete
        }

        @Override
        public long getCreationTime() {
            return (this.session != null) ? this.session.getCreationTime() : 0L;
        }

        @Override
        public long getLastAccessedTime() {
            return (this.session != null) ? this.session.getLastAccessedTime() : 0L;
        }
        ...
        @Override
        public int getMaxInactiveInterval() {
            return (this.session != null) ? this.session.getMaxInactiveInterval() : 0;
        }
        ...
        @Override
        public Set<String> getAttributeNames() {
            return (this.session != null) ? this.session.getAttributeNames() : Set.of();
        }

        @Override
        public Object setAttribute(String name, Object value) {
            return (this.session != null) ? this.session.setAttribute(name, value) : null;
        }

        @Override
        public Object removeAttribute(String name) {
            return (this.session != null) ? this.session.removeAttribute(name) : null;
        }
        ...
        @Override
        public String changeSessionId(HttpServerExchange exchange, SessionConfig config) {
            return (this.session != null) ? this.session.changeSessionId(exchange, config) : null;
        }

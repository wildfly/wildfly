 ...
 */
public class OneOffPatchXsd11TestCase extends AbstractTaskTestCase {
    ...
    public void apply2OneOffPatchesAndRollbackTheFirstOne() throws Exception {
        ...
        element.addContentModification(moduleAdded);
        element.addContentModification(standaloneShellUpdated);
        ...
        element = new PatchElementImpl("patch element 02");
        builder.addElement(element);
        element.setDescription("patch element 02 description");
        element.setNoUpgrade();

        provider = new PatchElementProviderImpl("base", "4.5.6", false);
        element.setProvider(provider);

        element.addContentModification(domainShellUpdated);

        Patch patch_2 = builder.build();

...
import static org.jboss.as.modcluster.ModClusterSSLResourceDefinition.KEYSTORE_PASSWORD;
...
import static org.jboss.as.modcluster.ModClusterSSLResourceDefinition.TRUSTSTORE_PASSWORD;
...
class ModClusterSubsystemAdd extends AbstractAddStepHandler {
    ...
    private ModClusterConfig getModClusterConfig(final OperationContext context, ModelNode model) throws OperationFailedException {
        ...
        if (model.get(SSL_CONFIGURATION_PATH.getKeyValuePair()).isDefined()) {
            ...
            ModelNode keystorePassword = KEYSTORE_PASSWORD.resolveModelAttribute(context, ssl);
            ModelNode truststorePassword = TRUSTSTORE_PASSWORD.resolveModelAttribute(context, ssl);
            ...
            if (truststorePassword.isDefined()) {
                config.setSslTrustStorePassword(truststorePassword.asString());
            } else if (password.isDefined()) {
                config.setSslTrustStorePassword(password.asString());
            }
            if (keystorePassword.isDefined()) {
                config.setSslKeyStorePassword(keystorePassword.asString());
            } else if (password.isDefined()) {

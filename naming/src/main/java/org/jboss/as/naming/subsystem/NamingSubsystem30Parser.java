...
class NamingSubsystem30Parser implements XMLElementReader<List<ModelNode>> {

    private final NamingSubsystemNamespace validNamespace;

    NamingSubsystem30Parser() {
        this.validNamespace = NamingSubsystemNamespace.NAMING_3_0;
    }

    NamingSubsystem30Parser(NamingSubsystemNamespace validNamespace) {
    ...
    private void parseBindings(final XMLExtendedStreamReader reader, final List<ModelNode> operations, PathAddress address) throws XMLStreamException {
        ...
        while (reader.hasNext() && reader.nextTag() != XMLStreamConstants.END_ELEMENT) {
            switch (NamingSubsystemXMLElement.forName(reader.getLocalName())) {
                ...
                case PROPERTIES: {
                    this.parsePropertiesBinding(reader, operations, address);
                    break;
                }
                default: {
                    throw unexpectedElement(reader);
    ...
    private void parsePropertiesBinding(XMLExtendedStreamReader reader, List<ModelNode> operations, PathAddress address) throws XMLStreamException {
        ...
        bindingAdd.get(OP_ADDR).set(address.append(BINDING, name).toModelNode());
        while (reader.hasNext() && reader.nextTag() != XMLStreamConstants.END_ELEMENT) {
            switch (NamingSubsystemXMLElement.forName(reader.getLocalName())) {
                case ENVIRONMENT_PROPERTY: {
                    ModelNode propertyNode = new ModelNode();
                    parsePropertyBinding(reader, propertyNode);
    ...
    private void parsePropertyBinding(XMLExtendedStreamReader reader, ModelNode propertyNode) throws XMLStreamException {

        if (NamingSubsystemXMLElement.forName(reader.getLocalName()) == NamingSubsystemXMLElement.ENVIRONMENT_PROPERTY) {
            for (int i = 0; i < reader.getAttributeCount(); i++) {
                requireNoNamespaceAttribute(reader, i);
                final String value = reader.getAttributeValue(i);
                final NamingSubsystemXMLAttribute attribute = NamingSubsystemXMLAttribute.forName(reader.getAttributeLocalName(i));
                switch (attribute) {
                    case NAME:
                        NamingBindingResourceDefinition.NAME.parseAndSetParameter(value, propertyNode, reader);
                        break;
                    case VALUE:
                        NamingBindingResourceDefinition.VALUE.parseAndSetParameter(value, propertyNode, reader);
                        break;
                    case TYPE:
                        NamingBindingResourceDefinition.TYPE.parseAndSetParameter(value, propertyNode, reader);
                        break;
                    default:
                        throw unexpectedAttribute(reader, i);
                }

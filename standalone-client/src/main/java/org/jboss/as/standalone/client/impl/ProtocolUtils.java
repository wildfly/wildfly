...
class ProtocolUtils {
    ...
    static void writeRequestHeader(final DataOutput output) throws Exception {
        output.write(StandaloneClientProtocol.SIGNATURE);
        output.writeByte(StandaloneClientProtocol.VERSION_FIELD);
        output.writeInt(StandaloneClientProtocol.VERSION);
        output.writeInt(0);
        output.writeByte(StandaloneClientProtocol.SERVER_CONTROLLER_REQUEST);
    }

    static int readResponseHeader(final DataInput input) throws IOException {
        validateSignature(input);
        expectHeader(input, StandaloneClientProtocol.VERSION_FIELD);
    ...
    static void validateSignature(final DataInput input) throws IOException {
        ...
        if (!Arrays.equals(StandaloneClientProtocol.SIGNATURE, signatureBytes)) {

...
import static org.jboss.as.standalone.client.impl.ProtocolUtils.*;
...
abstract class Request<T> extends AbstractMessageHandler {
    ...
    void handle(final Connection connection, final InputStream input) throws IOException {
        try {
            expectHeader(input, StandaloneClientProtocol.RESPONSE_START);
    ...
    protected void sendRequest(final int protocolVersion, final Connection connection) throws IOException {
        ...
        try {
            ...
            output.writeByte(StandaloneClientProtocol.REQUEST_OPERATION);
            output.writeByte(getRequestCode());
            output.writeByte(StandaloneClientProtocol.REQUEST_START);
            output.close();
            outputStream.close();
        } finally {
            safeClose(output);
        ...
        try {
            outputStream = connection.writeMessage();
            output = new SimpleByteDataOutput(outputStream);
            output.writeByte(StandaloneClientProtocol.REQUEST_END);
            output.close();
            outputStream.close();
        } finally {
            safeClose(output);
    ...
    private MessageHandler responseEndHandler = new AbstractMessageHandler() {
        @Override
        void handle(final Connection connection, final InputStream input) throws IOException {
            expectHeader(input, StandaloneClientProtocol.RESPONSE_END);

...
public class ManagedThreadFactoryImpl implements ManagedThreadFactory {
    ...
    ManageableThreadImpl newManageableThread(Runnable r) {
        if (contextConfiguration != null) {
            r = contextConfiguration.newManageableThreadContextualRunnable(r);
        }
        // new threads done on the thread factory class loader, avoiding possible leaks of class loaders
        final ClassLoader previous = WildFlySecurityManager.getCurrentContextClassLoaderPrivileged();
        WildFlySecurityManager.setCurrentContextClassLoaderPrivileged(this.getClass());
        try {
            return new ManageableThreadImpl(r, this);
        } finally {

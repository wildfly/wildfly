...
public class Util {
    ...
    public static void insertAnyPrimitive(Any any, Object primitive) {
        ...
        if (type == Boolean.class)
            ...
            throw IIOPMessages.MESSAGES.notAPrimitive(type.getName());
    }
    ...
    public static String javaToIDLName(String name) {
        if (name == null || "".equals(name) || name.indexOf('.') != -1)
            throw IIOPMessages.MESSAGES.nameCannotBeNullEmptyOrQualified();
    ...
    public static String getIRIdentifierOfClass(Class cls) {
        if (cls.isPrimitive())
            throw IIOPMessages.MESSAGES.primitivesHaveNoIRIds();
    ...
    static long getClassHashCode(Class cls) {
        ...
        try {
            ...
            throw IIOPMessages.MESSAGES.unavailableSHADigest(ex);
        }
        digest.update(bytes);
    ...
    private static String getSignature(Class cls) {
        ...
        if (cls.isPrimitive()) {
            ...
            if (cls == Boolean.TYPE)
                return "Z";
            throw IIOPMessages.MESSAGES.unknownPrimitiveType(cls.getName());
        }

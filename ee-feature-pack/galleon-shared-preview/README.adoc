= Use of wildfly-ee-feature-pack-galleon-shared-preview
:icons: font
...
The `wildfly-ee-feature-pack-galleon-shared-preview` module is used to control the source code and dependency management for items that
are relevant to *both* the `wildfly-ee` and `wildfly-preview` feature packs.
...
* If it's only for `wildfly-legacy-ee`, use the `wildfly-preview-feature-pack-galleon-local` maven module.
====

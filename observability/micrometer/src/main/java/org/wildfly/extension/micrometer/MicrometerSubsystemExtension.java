...
public class MicrometerSubsystemExtension implements Extension {
    ...
    static final ParentResourceDescriptionResolver SUBSYSTEM_RESOLVER = new SubsystemResourceDescriptionResolver(SUBSYSTEM_NAME, MicrometerSubsystemExtension.class);
    ...
    public void initializeParsers(ExtensionParsingContext context) {
        for (MicrometerSubsystemSchema schema : EnumSet.allOf(MicrometerSubsystemSchema.class)) {
            XMLElementReader<List<ModelNode>> reader = (schema == MicrometerSubsystemSchema.CURRENT) ? new PersistentResourceXMLDescriptionReader(this.currentDescription) : schema;

 ...
 * @author emartins
 */
public class ConcurrentEESubsystemParser20 {
    private ConcurrentEESubsystemParser20() {
    }
    ...
    static void parseManagedExecutorService(XMLExtendedStreamReader reader, List<ModelNode> operations, PathAddress subsystemPathAddress) throws XMLStreamException {
        ...
        final int count = reader.getAttributeCount();
        String name = null;
        final EnumSet<ConcurrentAttribute> required = EnumSet.of(ConcurrentAttribute.NAME, ConcurrentAttribute.JNDI_NAME, ConcurrentAttribute.CORE_THREADS);
        for (int i = 0; i < count; i++) {
            requireNoNamespaceAttribute(reader, i);
            final String value = reader.getAttributeValue(i);
            final ConcurrentAttribute attribute = ConcurrentAttribute.forName(reader.getAttributeLocalName(i));

 ...
 */
public class ServiceComponentInstantiator implements ManagedReferenceFactory {
    ...
    public ServiceComponentInstantiator(DeploymentUnit deploymentUnit, ComponentDescription componentDescription) {
        serviceName = deploymentUnit.getServiceName().append("component").append(componentDescription.getComponentName())
                .append("START");
    ...
    public ManagedReference getReference() {
        throw new UnsupportedOperationException("instatior does not creates instances, it only initializes");
    }
    ...
    private void setupComponent() {
        if (component == null) {
            synchronized (this) {
                if (component == null) {
                    component = ((ServiceController<BasicComponent>) serviceRegistry.getRequiredService(serviceName))
                            .getValue();
                }
            }
        }

    ...
    public void deploy(final DeploymentPhaseContext phaseContext) throws DeploymentUnitProcessingException {
        ...
        for (final InterceptorBindingMetaData containerInterceptorBinding : containerInterceptorBindings) {
            if (containerInterceptorBinding.getEjbName().equals("*")) {
                ...
                try {
                    pattern = Pattern.compile(containerInterceptorBinding.getEjbName());
                } catch (PatternSyntaxException e) {
                    throw new DeploymentUnitProcessingException("Invalid regular expression in ejb-name container interceptor binding: '"
                            + containerInterceptorBinding.getEjbName() + "': " + e.getMessage(), e);
                }

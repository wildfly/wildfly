 ...
 */
final class AssociationImpl implements Association, Registry.Listener<String, List<ClientMapping>>, AutoCloseable {
    ...
    AssociationImpl(final DeploymentRepository deploymentRepository, final Registry<String, List<ClientMapping>> clientMappingRegistry) {
        ...
        if (this.clientMappingRegistry != null) this.clientMappingRegistry.addListener(this);
    }

    @Override
    public void close() {
        if (this.clientMappingRegistry != null) this.clientMappingRegistry.removeListener(this);
    ...
    public ListenerHandle registerClusterTopologyListener(@NotNull final ClusterTopologyListener clusterTopologyListener) {
        // If there is no client mappings registry, then there is nothing with which to register
        if (this.clientMappingRegistry == null) return () -> {};
        ...
        synchronized (clusterTopologyListener) {
            ...
            clusterTopologyListener.clusterTopology(Collections.singletonList(getClusterInfo(this.clientMappingRegistry.getEntries())));
        }
        return () -> this.clusterTopologyListeners.remove(clusterTopologyListener);
    }

...
public class MdbValidationsUtilTest {
    ...
    public void mdbWithFinalClass() throws IOException, DeploymentUnitProcessingException {
        assertEquals(MdbValidityStatus.MDB_CLASS_CANNOT_BE_PRIVATE_ABSTRACT_OR_FINAL, MdbValidationsUtil.assertMDBClassValidity(buildClassInfoForClass(InvalidMdbFinalClass.class.getName())));
    }

    @Test
    public void mdbWithInterface() throws IOException, DeploymentUnitProcessingException {
        assertEquals(MdbValidityStatus.MDB_CANOT_BE_AN_INTERFACE, MdbValidationsUtil.assertMDBClassValidity(buildClassInfoForClass(InvaldMdbInterface.class.getName())));
    }

    @Test
    public void mdbWithFinalOnMessageMethod() throws IOException, DeploymentUnitProcessingException {
        assertEquals(MdbValidityStatus.MDB_ON_MESSAGE_METHOD_CANT_BE_FINAL, MdbValidationsUtil.assertMDBClassValidity(buildClassInfoForClass(InvalidMdbFinalMethods.class.getName())));
    }

    @Test
    public void mdbWithFinalizeMethod() throws IOException, DeploymentUnitProcessingException {
        assertEquals(MdbValidityStatus.MDB_SHOULD_NOT_HAVE_FINALIZE_METHOD, MdbValidationsUtil.assertMDBClassValidity(buildClassInfoForClass(InvalidMdbWithFinalizeMethod.class.getName())));
    }
